...
export function TempMailModal({ modalProps }: { modalProps: RenderModalProps; }) {
    ...
    async function fetchInbox(acc: SavedAccount, silent = false) {
        ...
        try {
            ...
            if (!silent) setError(t("╪¬╪╣╪░┘æ╪▒ ╪¬╪¡┘à┘è┘ä ╪º┘ä┘ê╪º╪▒╪»: ", "Failed to load inbox: ") + e.message);
        } finally {
    ...
    async function openMessage(msg: TmMessage) {
        ...
        try {
            ...
            setError(t("╪¬╪╣╪░┘æ╪▒ ╪¬╪¡┘à┘è┘ä ╪º┘ä╪▒╪│╪º┘ä╪⌐: ", "Could not load message: ") + e.message);
        } finally {
            setMsgLoading(false);
        }
    }
    ...
    async function createAddress(random: boolean) {
        ...
        if (!domain) { setError(t("┘ä╪º ╪¬┘ê╪¼╪» ┘å╪╖╪º┘é╪º╪¬ ┘à╪¬╪º╪¡╪⌐ ╪¿╪╣╪».", "No domains available yet.")); return; }
        const user = random ? randomString(10) : customUser.trim();
        if (!user) { setError(t("╪ú╪»╪«┘ä ╪º╪│┘à ┘à╪│╪¬╪«╪»┘à ╪ú┘ê┘ä╪º┘ï.", "Enter a username first.")); return; }
        ...
        try {
            ...
            setView("inbox");
            setCustomUser("");
            showToast(t("╪¬┘à ╪º┘ä╪Ñ┘å╪┤╪º╪í: ", "Created: ") + address, Toasts.Type.SUCCESS);
        } catch (e: any) {
            setError(e.message);
        } finally {
            setLoading(false);
    ...
    return (
        <Modal {...modalProps} size="lg" title={t("╪º┘ä╪¿╪▒┘è╪» ╪º┘ä┘à╪ñ┘é╪¬", "Temp Mail")}>
            <div className="tm-shell">

                {/* ΓöÇΓöÇ Sidebar ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
                <div className="tm-sidebar">
                    <div className="tm-brand">
                        ...
                        <span>{t("╪º┘ä╪¿╪▒┘è╪» ╪º┘ä┘à╪ñ┘é╪¬", "Temp Mail")}</span>
                    </div>
                    ...
                    {active && (
                        <div className="tm-active-addr">
                            ...
                            <button className="tm-copy-btn" onClick={() => { copyToClipboard(active.address); showToast(t("╪¬┘à ╪º┘ä┘å╪│╪«!", "Copied!"), Toasts.Type.SUCCESS); }}>
                                Copy address
                            </button>
                        </div>
                    )}
                    ...
                    <nav className="tm-nav">
                        <button className={`tm-nav-item ${view === "inbox" ? "tm-active" : ""}`} onClick={() => setView("inbox")}>
                            <span className="tm-nav-icon">≡ƒôÑ</span>
                            <span>{t("╪º┘ä┘ê╪º╪▒╪»", "Inbox")}</span>
                ...
                {/* ΓöÇΓöÇ Content ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
                <div className="tm-content">
                    {error && (
                        <div className="tm-error-bar">
                            <span>ΓÜá {error}</span>
                            <button onClick={() => setError("")}>Γ£ò</button>
                        </div>
                    )}
                    ...
                    {view === "inbox" && (
                        <div className="tm-view">
                            <div className="tm-view-header">
                                ...
                                <button className="tm-icon-btn" title={t("╪¬╪¡╪»┘è╪½", "Refresh")} onClick={() => active && fetchInbox(active)} disabled={loading}>
                            ...
                            {!active && (
                                <div className="tm-empty-state">
                                    <div className="tm-empty-icon">Γ£ë</div>
                            ...
                            {active && loading && <div className="tm-spinner">LoadingΓÇª</div>}

                            {active && !loading && messages.length === 0 && (
                                <div className="tm-empty-state">
                                    <div className="tm-empty-icon">≡ƒô¡</div>
                                    ...
                                    {messages.map(m => (
                                        <div key={m.id} className={`tm-msg-row ${!m.seen ? "tm-msg-unread" : ""}`} onClick={() => openMessage(m)}>
                                            ...
                                            <div className="tm-msg-right">
                                                ...
                                                <button className="tm-del-btn" title={t("╪¡╪░┘ü", "Delete")} onClick={e => { e.stopPropagation(); handleDeleteMessage(m.id); }}>
                    ...
                    {view === "accounts" && (
                        <div className="tm-view">
                            ...
                            {accounts.length === 0 && (
                                <div className="tm-empty-state">
                                    <div className="tm-empty-icon">≡ƒæñ</div>
                                ...
                                {accounts.map(acc => (
                                    <div key={acc.id} className={`tm-account-card ${active?.id === acc.id ? "tm-account-active" : ""}`}>
                                        ...
                                        <div className="tm-account-btns">
                                            ...
                                            <button className="tm-icon-btn" title={t("┘å╪│╪«", "Copy")} onClick={() => { copyToClipboard(acc.address); showToast(t("╪¬┘à ╪º┘ä┘å╪│╪«!", "Copied!"), Toasts.Type.SUCCESS); }}>
                                            ...
                                            <button className="tm-icon-btn tm-icon-btn-danger" title={t("╪¡╪░┘ü ╪º┘ä╪¡╪│╪º╪¿", "Delete account")} onClick={() => deleteAcc(acc)}>
                                                <svg viewBox="0 0 24 24" width={14} height={14} fill="currentColor"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z" /></svg>
                                            </button>
                                        </div>
                                    </div>
                        ...
                        <div className="tm-view">
                            ...
                            <div className="tm-new-section">
                                ...
                                <button className="tm-btn-primary" onClick={() => createAddress(true)} disabled={loading || !domains.length}>
                                    {loading ? t("╪¼╪º╪▒┘è ╪º┘ä╪Ñ┘å╪┤╪º╪íΓÇª", "CreatingΓÇª") : t("ΓÜí ╪¬┘ê┘ä┘è╪» ╪╣┘å┘ê╪º┘å ╪╣╪┤┘ê╪º╪ª┘è", "ΓÜí Generate random address")}
                                </button>
                            </div>

                            <div className="tm-divider"><span>{t("╪ú┘ê ╪º╪«╪¬╪▒ ╪╣┘å┘ê╪º┘å┘â", "or choose your own")}</span></div>

                            <div className="tm-new-section">
                                <div className="tm-new-label">Custom address</div>
                                <div className="tm-custom-input-row">
                                    <TextInput
                                        placeholder={t("╪º╪│┘à ╪º┘ä┘à╪│╪¬╪«╪»┘à", "username")}
                                        value={customUser}
                                ...
                                >
                                    {loading ? t("╪¼╪º╪▒┘è ╪º┘ä╪Ñ┘å╪┤╪º╪íΓÇª", "CreatingΓÇª") : t("╪Ñ┘å╪┤╪º╪í ╪╣┘å┘ê╪º┘å", "Create address")}
                                </button>
                            </div>
                        </div>
                    )}
