...
// ΓöÇΓöÇ Settings ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

const settings = definePluginSettings({
    wallpapers: {
        type: OptionType.STRING,
        description: "Wallpapers JSON ΓÇö do not modify manually (managed by plugin)",
        default: "{}",
        hidden: true,
        restartNeeded: false,
...
// Fully local ΓÇö the wallpaper is only ever stored in your own settings and rendered in your
...
// ΓöÇΓöÇ File / URL pickers ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function promptUrl(): Promise<string | null> {
    return new Promise(resolve => {
        const url = prompt(t("╪ú╪»╪«┘ä ╪▒╪º╪¿╪╖ ╪º┘ä╪╡┘ê╪▒╪⌐ ╪ú┘ê GIF ╪ú┘ê ╪º┘ä┘ü┘è╪»┘è┘ê:", "Enter the URL for the image, gif, or video:"));
        resolve(url?.trim() || null);
    });
}

// ΓöÇΓöÇ CSS Injection ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
// ΓöÇΓöÇ Context menu actions ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

async function setWallpaperFromFile(channelId: string) {
    const file = await pickFileRaw();
    if (!file) return;

    // Stored locally as a data URL ΓÇö never uploaded anywhere.
    const reader = new FileReader();
    reader.onload = () => {
        saveWallpaper(channelId, reader.result as string);
        showToast(t("╪¬┘à ╪¬╪╖╪¿┘è┘é ╪º┘ä╪«┘ä┘ü┘è╪⌐ (┘à╪¡┘ä┘è╪º┘ï)", "Wallpaper applied (locally)"), Toasts.Type.SUCCESS);
    };
...
async function setWallpaperFromUrl(channelId: string) {
    ...
    if (url) {
        ...
        showToast(t("╪¬┘à ╪¬╪╖╪¿┘è┘é ╪º┘ä╪«┘ä┘ü┘è╪⌐", "Wallpaper applied"), Toasts.Type.SUCCESS);
    }
}

function removeWallpaper(channelId: string) {
    saveWallpaper(channelId, "");
    showToast(t("╪¬┘à ╪¡╪░┘ü ╪º┘ä╪«┘ä┘ü┘è╪⌐", "Wallpaper deleted"), Toasts.Type.SUCCESS);
}

// ΓöÇΓöÇ Context Menu Patches ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function buildWallpaperMenu(channelId: string): React.ReactElement {
    ...
    return (
        <Menu.MenuItem
            ...
            label={t("╪«┘ä┘ü┘è╪⌐", "Wallpaper")}
            icon={WallpaperIcon}
        >
            <Menu.MenuItem
                id="wallpaper-from-file"
                label={t("≡ƒôü ┘à┘å ┘à┘ä┘ü...", "≡ƒôü From a file...")}
                action={() => setWallpaperFromFile(channelId)}
            />
            <Menu.MenuItem
                id="wallpaper-from-url"
                label={t("≡ƒöù ┘à┘å ╪▒╪º╪¿╪╖...", "≡ƒöù From a URL...")}
            ...
            {has && (
                <>
                    ...
                    <Menu.MenuItem
                        id="wallpaper-remove"
                        label={t("≡ƒùæ∩╕Å ╪¡╪░┘ü ╪º┘ä╪«┘ä┘ü┘è╪⌐", "≡ƒùæ∩╕Å Delete wallpaper")}
                        color="danger"
...
export default definePlugin({
    ...
    description: "Allows for custom backgrounds for every individual channel. Fully local ΓÇö nothing is uploaded or shared.",
    settings,
