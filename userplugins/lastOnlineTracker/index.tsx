...
const settings = definePluginSettings({
    persist: {
        ...
        default: false,
        description: "Keep last-seen times after restart. Off by default ΓÇö saved times don't refresh until that person goes offline again, so they can go stale."
    }
});
...
function activeAgo(ms: number) {
    return t(`┘å╪┤┘É╪╖ ┘à┘å╪░ ${ago(ms)}`, `Active ${ago(ms)} ago`);
}
...
const ctxPatch = (children: any[], props: any) => {
    ...
    group.push(
        ...
        <Menu.MenuItem key="los-item" id="los-item" disabled
            ...
            subtext={t(`╪ó╪«╪▒ ╪╕┘ç┘ê╪▒: ${new Date(ts).toLocaleString()}`, `Last online: ${new Date(ts).toLocaleString()}`)} />
    );
};
