...
function GifIcon({ active }: IconProps & { active?: boolean; }) {
    return (
...
// ΓöÇΓöÇΓöÇ Popover component ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function GifConvertorPopover({ position, onClose }: PopoverProps) {
    ...
    async function processFile(file: File) {
        if (!file.type.startsWith("image/") && !file.type.startsWith("video/")) {
            setErrorMsg(t("┘è┘Å╪»╪╣┘Ä┘à ╪º┘ä╪╡┘ê╪▒ ┘ê╪º┘ä┘ü┘è╪»┘è┘ê ┘ü┘é╪╖.", "Only image and video files are supported."));
            setStage("error");
            return;
        }
        ...
        try {
            ...
            setErrorMsg(e?.message ?? t("┘ü╪┤┘ä ╪º┘ä╪¬╪¡┘ê┘è┘ä.", "Conversion failed."));
            setStage("error");
        }
    }
    ...
    async function handleSend() {
        ...
        try {
            ...
            showToast(t("╪¬┘à ╪Ñ╪▒╪│╪º┘ä GIF!", "GIF sent!"), Toasts.Type.SUCCESS);
            onClose();
        } catch (e: any) {
            console.error("[GifConvertor] Upload error:", e);
            setErrorMsg(e?.message ?? t("┘ü╪┤┘ä ╪º┘ä╪▒┘ü╪╣.", "Upload failed."));
            setStage("error");
        }
    }
    ...
    return (
        ...
        >
            ...
            <div className="nc-gifconv-header">
                <span className="nc-gifconv-title">Γ£ª GIF Convertor</span>
            ...
            {(stage === "idle" || stage === "error") && (
                <>
                    <div
                        ...
                        aria-label={t("╪ú┘ü┘ä┘É╪¬ ┘ê╪│╪º╪ª╪╖ ┘ä╪¬╪¡┘ê┘è┘ä┘ç╪º ╪Ñ┘ä┘ë GIF", "Drop media to convert to GIF")}
                    >
                        <span className="nc-gifconv-dropzone-icon">
                            <UploadImageIcon />
                        </span>
                        <span className="nc-gifconv-dropzone-label">
                            {isDragOver ? t("╪ú┘ü┘ä┘É╪¬ ┘ä┘ä╪¬╪¡┘ê┘è┘ä!", "Release to convert!") : t("╪ú┘ü┘ä┘É╪¬ ╪º┘ä┘ê╪│╪º╪ª╪╖ ┘ç┘å╪º", "Drop media here")}
                        </span>
                        <span className="nc-gifconv-dropzone-sub">
                            or click to browse ┬╖ Ctrl+V to paste<br />
                            Images &amp; Videos (MP4, WebMΓÇª)
                        </span>
                    </div>
                    {stage === "error" && (
                        <div className="nc-gifconv-error">ΓÜá {errorMsg}</div>
                    )}
                </>
            )}
            ...
            {stage === "converting" && (
                <div className="nc-gifconv-loading">
                    ...
                    <span className="nc-gifconv-loading-label">
                        Converting to GIFΓÇª {progress > 0 && `(${Math.round(progress * 100)}%)`}
                    </span>
                </div>
            )}
            ...
            {stage === "sending" && (
                <div className="nc-gifconv-loading">
                    <div className="nc-gifconv-spinner" />
                    <span className="nc-gifconv-loading-label">UploadingΓÇª</span>
                </div>
            )}
        </div>
    );
...
// ΓöÇΓöÇΓöÇ ChatBar button ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    return (
        <>
            <span ref={btnWrapRef} className="nc-gifconv-btn-wrap">
                <ChatBarButton tooltip={t("╪¡┘ê┘æ┘ä ╪╡┘ê╪▒╪⌐ ╪ú┘ê ┘ü┘è╪»┘è┘ê ╪Ñ┘ä┘ë GIF", "Convert image or video to GIF")} onClick={toggle}>
                    <GifIcon active={open} />
                </ChatBarButton>
            </span>
...
// ΓöÇΓöÇΓöÇ Plugin definition ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
