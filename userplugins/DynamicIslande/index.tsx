...
const cl = classNameFactory("vc-esharq-dynamic-island-");
const NOTIFICATION_DURATION = 5000;
const RUNTIME_KEY = Symbol.for("EsharqDynamicIsland.runtime");
...
const settings = definePluginSettings({
    islandColor: {
        ...
        options: [
            { label: t("╪┤┘ü┘æ╪º┘ü", "Transparent"), value: "transparent" },
            { label: t("╪½┘è┘à ╪»┘è╪│┘â┘ê╪▒╪»", "Discord theme"), value: "theme" },
            { label: t("╪ú╪│┘ê╪» (AMOLED)", "AMOLED"), value: "amoled", default: true },
            { label: t("╪ú╪¿┘è╪╢", "White"), value: "white" },
            { label: t("╪ú╪▓╪▒┘é ┘ü╪º╪¬╪¡", "Light blue"), value: "blue" },
            { label: t("┘ê╪▒╪»┘è", "Pink"), value: "pink" }
        ]
    },
    keepIslandVisible: {
...
function SpotifySection() {
    ...
    return (
        <section className={cl("section")} aria-label={t("╪╣┘å╪º╪╡╪▒ ╪¬╪¡┘â┘æ┘à Spotify", "Spotify controls")}>
            ...
            <div className={cl("controls")}>
                <ControlButton label={t("╪º┘ä┘à┘é╪╖╪╣ ╪º┘ä╪│╪º╪¿┘é", "Previous track")} onClick={() => SpotifyStore.prev()}>
                    <Glyph path="M6 5h2v14H6V5Zm3 7 9-7v14l-9-7Z" />
                </ControlButton>
                <ControlButton label={isPlaying ? t("╪Ñ┘è┘é╪º┘ü ┘à╪ñ┘é┘æ╪¬", "Pause") : t("╪¬╪┤╪║┘è┘ä", "Play")} active={isPlaying} onClick={() => SpotifyStore.setPlaying(!isPlaying)}>
                    <Glyph path={isPlaying ? "M6 5h4v14H6V5Zm8 0h4v14h-4V5Z" : "M8 5v14l11-7L8 5Z"} />
                </ControlButton>
                <ControlButton label={t("╪º┘ä┘à┘é╪╖╪╣ ╪º┘ä╪¬╪º┘ä┘è", "Next track")} onClick={() => SpotifyStore.next()}>
...
function ParticipantRow({ p, channel, currentUser }: { p: { user: any; member: any; }; channel: any; currentUser: any; }) {
    ...
    return (
        <div
            className="vc-esharq-dynamic-island-participant-row"
            onContextMenu={e => {
                e.preventDefault();
            ...
            title={t("╪º┘å┘é╪▒ ╪¿╪º┘ä╪▓╪▒ ╪º┘ä╪ú┘è┘à┘å ┘ä┘å╪│╪« ╪º┘ä┘à╪╣╪▒┘æ┘ü", "Right click to copy ID")}
        >
            ...
            {showParticipantButtons && (
                ...
                >
                    <ControlButton
                        label={isMuted ? t("╪Ñ┘ä╪║╪º╪í ╪º┘ä┘â╪¬┘à", "Unmute") : t("┘â╪¬┘à", "Mute")}
                        danger={isMuted}
                        compact
                        onClick={() => {
                    ...
                    {!isSelf && (
                        <>
                            <ControlButton
                                label={isFollowingUser ? t("╪Ñ┘ä╪║╪º╪í ╪º┘ä┘à╪¬╪º╪¿╪╣╪⌐", "Unfollow") : t("┘à╪¬╪º╪¿╪╣╪⌐", "Follow")}
                                compact
                            ...
                            <ControlButton
                                label={isFriend ? t("╪Ñ╪▓╪º┘ä╪⌐ ╪╡╪»┘è┘é", "Remove Friend") : (isOutgoing || isIncoming ? t("╪Ñ┘ä╪║╪º╪í ╪º┘ä╪╖┘ä╪¿", "Cancel Request") : t("╪Ñ╪╢╪º┘ü╪⌐ ╪╡╪»┘è┘é", "Add Friend"))}
                                compact
                                onClick={() => {
                            ...
                            <ControlButton label={t("╪Ñ╪▒╪│╪º┘ä ╪▒╪│╪º┘ä╪⌐", "Send DM")} compact onClick={() => ChannelActionCreators.openPrivateChannel({ recipientIds: [p.user.id] })}>
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
...
function VoiceSection({ channelId }: { channelId: string; }) {
    ...
    return (
        <div className={cl("section")} style={{ display: "flex", flexDirection: "column", alignItems: "stretch", padding: 0, gap: 0 }}>
            <div aria-label={t("╪╣┘å╪º╪╡╪▒ ╪¬╪¡┘â┘æ┘à ┘à┘â╪º┘ä┘à╪⌐ ╪»┘è╪│┘â┘ê╪▒╪»", "Discord call controls")} style={{ display: "flex", minWidth: 0, padding: "8px", alignItems: "center", gap: "10px" }}>
                <div
                    ...
                    title={showCallParticipants ? t("╪º┘å┘é╪▒ ┘ä╪Ñ╪╕┘ç╪º╪▒/╪Ñ╪«┘ü╪º╪í ┘é╪º╪ª┘à╪⌐ ╪º┘ä┘à╪┤╪º╪▒┘â┘è┘å", "Click to toggle participants list") : undefined}
                >
                    <div className={cl("call-indicator")}><span /></div>
                    <div className={cl("copy")}>
                        <strong>{channel?.name || t("┘à┘â╪º┘ä┘à╪⌐ ╪»┘è╪│┘â┘ê╪▒╪»", "Discord call")}</strong>
                        <span>{participantCount} {participantCount === 1 ? t("┘à╪┤╪º╪▒┘â", "participant") : t("┘à╪┤╪º╪▒┘â┘ê┘å", "participants")}</span>
                    </div>
                </div>
                {showCallControls && (
                    <div className={cl("controls")}>
                        <ControlButton label={isMuted ? t("╪Ñ┘ä╪║╪º╪í ╪º┘ä┘â╪¬┘à", "Unmute") : t("┘â╪¬┘à", "Mute")} danger={isMuted} onClick={() => IslandVoiceActions.toggleSelfMute()}>
                            <VoiceIcon slashed={isMuted}><Microphone /></VoiceIcon>
                        </ControlButton>
                        <ControlButton label={isDeafened ? t("╪Ñ┘ä╪║╪º╪í ╪º┘ä╪Ñ╪╡┘à╪º┘à", "Undeafen") : t("╪Ñ╪╡┘à╪º┘à", "Deafen")} danger={isDeafened} onClick={() => IslandVoiceActions.toggleSelfDeaf()}>
                            <VoiceIcon slashed={isDeafened}><HeadphonesIcon /></VoiceIcon>
                        </ControlButton>
                        <ControlButton label={t("┘é╪╖╪╣ ╪º┘ä╪º╪¬╪╡╪º┘ä", "Disconnect")} danger onClick={() => IslandChannelActions.selectVoiceChannel(null)}>
            ...
            {showCallParticipants && expanded && participants.length > 0 && (
                <div
                    className="vc-esharq-dynamic-island-voice-participants-list"
...
function ScreenShareSection({ startedAt, stream }: { startedAt: number; stream: Stream; }) {
    ...
    return (
        <section className={cl("section", "screen-section")} aria-label={t("╪╣┘å╪º╪╡╪▒ ╪¬╪¡┘â┘æ┘à ┘à╪┤╪º╪▒┘â╪⌐ ╪º┘ä╪┤╪º╪┤╪⌐", "Screen sharing controls")}>
            <div className={cl("section-info")}>
                <div className={cl("stream-indicator")}><ScreenShareIcon /></div>
                <div className={cl("copy")}>
                    <strong>{channel.name || t("┘à╪┤╪º╪▒┘â╪⌐ ╪º┘ä╪┤╪º╪┤╪⌐", "Screen sharing")}</strong>
                    <span><ScreenShareTimer startedAt={startedAt} /> ┬╖ {viewerCount} {viewerCount === 1 ? t("┘à╪┤╪º┘ç╪»", "viewer") : t("┘à╪┤╪º┘ç╪»┘ê┘å", "viewers")}</span>
                </div>
            </div>
            <div className={cl("controls")}>
                <ControlButton label={t("╪Ñ┘è┘é╪º┘ü ╪º┘ä┘à╪┤╪º╪▒┘â╪⌐", "Stop sharing")} danger onClick={() => stopScreenShare(stream)}>
    ...
    useEffect(() => {
        ...
        const handleMessage = ({ message }: { message: MessageWithMentions; }) => {
            ...
            setIslandNotification({
                ...
                body: message.content.trim() || (message.attachments.length ? t("╪ú╪▒╪│┘ä ┘à┘Å╪▒┘ü┘é╪º┘ï.", "Sent an attachment.") : t("╪ú╪▒╪│┘ä ╪▒╪│╪º┘ä╪⌐.", "Sent a message.")),
                id: message.id,
    ...
    return (
        <div className={cl("root", `color-${islandColor}`, {
            ...
            <Clickable
                ...
                aria-label={t("╪º┘ä╪¼╪▓┘è╪▒╪⌐ ╪º┘ä╪»┘è┘å╪º┘à┘è┘â┘è╪⌐", "Dynamic Island")}
            ...
            >
                ...
                <div key={notification?.id ?? primary ?? "idle"} className={cl("summary-copy")}>
                    <strong>{notification?.title ?? (primaryStream ? t("╪ú┘å╪¬ ╪¬╪┤╪º╪▒┘â ╪┤╪º╪┤╪¬┘â", "You are sharing your screen") : primaryTrack?.name ?? (primaryChannelId ? t("┘à┘â╪º┘ä┘à╪⌐ ╪»┘è╪│┘â┘ê╪▒╪»", "Discord call") : t("╪º┘ä╪¼╪▓┘è╪▒╪⌐ ╪º┘ä╪»┘è┘å╪º┘à┘è┘â┘è╪⌐", "Dynamic Island")))}</strong>
                    <span>{notification?.body ?? (primaryStream
                        ? <>{t("┘à╪¿╪º╪┤╪▒ ┘à┘å╪░", "Live for")} <ScreenShareTimer startedAt={streamStartedAt} /></>
                        : primaryTrack
                            ? primaryTrack.artists.map(artist => artist.name).join(", ")
                            : primaryChannelId ? t("╪╣┘å╪º╪╡╪▒ ╪¬╪¡┘â┘æ┘à ╪º┘ä┘à┘â╪º┘ä┘à╪⌐ ┘à╪¬╪º╪¡╪⌐", "Call controls available") : t("╪¼╪º┘ç╪▓╪⌐ ┘ä╪ú┘å╪┤╪╖╪¬┘â", "Ready for your activities"))}</span>
                </div>
                {!notification && primaryTrack && (
                    <span className={cl("visualizer")} aria-label={primaryIsPlaying ? t("┘è╪╣┘à┘ä", "Playing") : t("┘à╪¬┘ê┘é┘æ┘ü", "Paused")}>
                        <span /><span /><span />
                    </span>
                )}
                {!notification && primaryStream && (
                    <ControlButton compact label={t("╪Ñ┘è┘é╪º┘ü ╪º┘ä┘à╪┤╪º╪▒┘â╪⌐", "Stop sharing")} danger onClick={() => stopScreenShare(primaryStream)}>
                        <Glyph path="M7 7h10v10H7V7Z" />
                    </ControlButton>
                )}
                {!notification && primaryChannelId && <span className={cl("live-dot")} aria-label={t("┘à┘â╪º┘ä┘à╪⌐ ┘å╪┤╪╖╪⌐", "Call active")} />}
                {!notification && activeIslands.length > 1 && (
                    <span className={cl("pages")} aria-label={t(`${activeIslands.length} ╪¼╪▓╪▒ ┘å╪┤╪╖╪⌐`, `${activeIslands.length} active Islands`)}>
            ...
            <div className={cl("panel-shell")} aria-hidden={!expanded}>
                <div className={cl("panel-clip")}>
                    <div className={cl("panel")}>
                        ...
                        {idle && <div className={cl("empty")}>{t("┘ü╪╣┘æ┘ä ┘å┘ê╪╣ ╪¼╪▓┘è╪▒╪⌐╪î ╪ú┘ê ╪┤╪║┘æ┘ä ┘à┘ê╪│┘è┘é┘ë╪î ╪ú┘ê ╪º┘å╪╢┘à┘æ ┘ä┘à┘â╪º┘ä┘à╪⌐ ┘ä╪Ñ╪╕┘ç╪º╪▒ ╪╣┘å╪º╪╡╪▒ ╪º┘ä╪¬╪¡┘â┘æ┘à.", "Enable an Island type, play music, or join a call to show controls.")}</div>}
                    </div>
                </div>
            </div>
        </div>
