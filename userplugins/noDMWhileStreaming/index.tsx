...
const settings = definePluginSettings({
    hideGroups: {
        ...
        description: t("╪Ñ╪«┘ü╪º╪í ╪º┘ä╪▒╪│╪º╪ª┘ä ╪º┘ä╪¼┘à╪º╪╣┘è╪⌐ ╪ú┘è╪╢╪º┘ï ╪ú╪½┘å╪º╪í ╪º┘ä╪¿╪½┘æ", "Hide Group DMs too while streaming"),
        default: false
    },
    debugMode: {
        type: OptionType.BOOLEAN,
        description: t("┘ê╪╢╪╣ ╪º┘ä╪¬╪╡╪¡┘è╪¡ ΓÇö ┘è╪╣╪▒╪╢ ╪│╪¼┘ä┘æ╪º╪¬ ┘à┘ü╪╡┘æ┘ä╪⌐ ┘ü┘è ╪º┘ä╪╖╪▒┘ü┘è╪⌐", "Debug Mode - Shows detailed logs in the console"),
        default: false
    }
});
...
export default definePlugin({
    ...
    // Flux events ΓÇö intercept MESSAGE_CREATE to auto-ack DMs/Groups during stream
    flux: {
    ...
    filterChannels(ids: string[]) {
        ...
        if (settings.store.debugMode) {
            console.log(`[NoDMWhileStreaming] [DEBUG] ≡ƒÄ¿ filterChannels called. IDs count: ${ids?.length}, Streaming: ${streaming}`);
        }
        ...
        if (settings.store.debugMode) {
            console.log(`[NoDMWhileStreaming] [DEBUG] ≡ƒÄ¿ filterChannels filtered. Remaining: ${filtered.length}`);
        }
    ...
    start() {
        ...
        if (typeof window !== "undefined" && window.Notification) {
            ...
            try {
                ...
                (window.Notification as any).prototype = originalNotification.prototype;
                (window.Notification as any).permission = originalNotification.permission;
                (window.Notification as any).requestPermission = originalNotification.requestPermission;
            } catch (e) {
