...
const settings = definePluginSettings({
    showToast: {
        ...
        description: t("╪╣╪▒╪╢ ╪Ñ╪┤╪╣╪º╪▒ ╪╣┘å╪» ╪Ñ╪▓╪º┘ä╪⌐ ┘ä┘é╪¿ ┘à┘ü╪▒┘ê╪╢", "Show a notification when a forced nickname is removed"),
        default: true,
    }
});
...
async function resetNick(guildId: string, forcedNick: string) {
    ...
    try {
        ...
        try {
            ...
            if (settings.store.showToast) showToast(t(`╪ú┘Å╪▓┘è┘ä ╪º┘ä┘ä┘é╪¿ ╪º┘ä┘à┘ü╪▒┘ê╪╢ ┬½${forcedNick}┬╗`, `Removed forced nickname "${forcedNick}"`), Toasts.Type.SUCCESS);
            return;
        ...
        if (settings.store.showToast) showToast(t(`╪¬╪╣╪░┘æ╪▒ ╪Ñ╪▓╪º┘ä╪⌐ ╪º┘ä┘ä┘é╪¿ (${err?.status ?? "?"})`, `Failed to reset nickname (${err?.status ?? "?"})`), Toasts.Type.FAILURE);
    } finally {
