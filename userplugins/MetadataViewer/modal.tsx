...
import { ModalCloseButton, ModalHeader, ModalRoot, type RenderModalProps } from "@utils/esharqModals";
...
const CopyButton = ({ text }: CopyButtonProps) => {
    ...
    const handleCopy = () => {
        ...
        setCopied(true);
        showToast(t("╪¬┘à ╪º┘ä┘å╪│╪« ╪Ñ┘ä┘ë ╪º┘ä╪¡╪º┘ü╪╕╪⌐!", "Copied to clipboard!"), Toasts.Type.SUCCESS);
    ...
    return (
        <button
            ...
            title={t("┘å╪│╪« ╪Ñ┘ä┘ë ╪º┘ä╪¡╪º┘ü╪╕╪⌐", "Copy to clipboard")}
            type="button"
        >
            {copied ? (
...
export function MetadataScannerModal({ rootProps, url, name, mimeType, size }: {
    ...
    const copyImageToClipboard = async () => {
        try {
            showToast(t("╪¼╪º╪▒┘ì ┘å╪│╪« ╪º┘ä╪╡┘ê╪▒╪⌐ΓÇª", "Copying image..."), Toasts.Type.MESSAGE);
            ...
            showToast(t("╪¬┘à ┘å╪│╪« ╪º┘ä╪╡┘ê╪▒╪⌐ ╪Ñ┘ä┘ë ╪º┘ä╪¡╪º┘ü╪╕╪⌐!", "Image copied to clipboard!"), Toasts.Type.SUCCESS);
        } catch (err) {
            logger.error("Failed to copy image to clipboard", err);
            showToast(t("╪¬╪╣╪░┘æ╪▒ ┘å╪│╪« ╪º┘ä╪╡┘ê╪▒╪⌐: ", "Failed to copy image: ") + String(err), Toasts.Type.FAILURE);
        }
    };
    ...
    const downloadImage = async () => {
        ...
        try {
            showToast(t("╪¼╪º╪▒┘ì ╪¬┘å╪▓┘è┘ä ╪º┘ä╪╡┘ê╪▒╪⌐ΓÇª", "Downloading image..."), Toasts.Type.MESSAGE);
            const response = await fetch(url);
            const blob = await response.blob();
            ...
            showToast(t("╪¿╪»╪ú ╪¬┘å╪▓┘è┘ä ╪º┘ä╪╡┘ê╪▒╪⌐!", "Image download started!"), Toasts.Type.SUCCESS);
        } catch (err) {
            logger.error("Failed to download image", err);
            showToast(t("╪¬╪╣╪░┘æ╪▒ ╪º┘ä╪¬┘å╪▓┘è┘ä: ", "Failed to download: ") + String(err), Toasts.Type.FAILURE);
        } finally {
    ...
    const tabs = useMemo(() => {
        const list = [{ id: "file", label: t("┘à╪╣┘ä┘ê┘à╪º╪¬ ╪º┘ä┘à┘ä┘ü", "File Info") }];
        if (data?.exif && Object.keys(data.exif).length > 0) {
            list.push({ id: "exif", label: t("┘ê╪│┘ê┘à EXIF", "EXIF Tags") });
        }
        if (data?.pngChunks && data.pngChunks.length > 0) {
            list.push({ id: "chunks", label: t("┘à┘é╪º╪╖╪╣ PNG", "PNG Chunks") });
        }
        if (data?.sdParams) {
            list.push({ id: "sd", label: t("┘à┘ê╪¼┘æ┘ç╪º╪¬ SD", "SD Prompts") });
        }
        if (data?.gps) {
            list.push({ id: "gps", label: t("GPS / ╪«╪▒┘è╪╖╪⌐", "GPS / Map") });
        }
        if (data && Object.keys(data.rawTags).length > 0) {
            list.push({ id: "raw", label: t("JSON ╪«╪º┘à", "Raw JSON") });
        }
        return list;
    }, [data]);
                ...
                >
                    ...
                    >
                        ...
                        <button
                            ...
                            onMouseEnter={e => { e.currentTarget.style.backgroundColor = "rgba(0, 0, 0, 0.8)"; }}
                            onMouseLeave={e => { e.currentTarget.style.backgroundColor = "rgba(0, 0, 0, 0.6)"; }}
                            title={t("╪¬┘å╪▓┘è┘ä ╪º┘ä╪╡┘ê╪▒╪⌐", "Download Image")}
                            type="button"
                        >
                        ...
                        <button
                            ...
                            onMouseEnter={e => { e.currentTarget.style.backgroundColor = "rgba(0, 0, 0, 0.8)"; }}
                            onMouseLeave={e => { e.currentTarget.style.backgroundColor = "rgba(0, 0, 0, 0.6)"; }}
                            title={t("┘à┘ä╪í ╪º┘ä╪┤╪º╪┤╪⌐", "Fullscreen")}
                            type="button"
                        >
                            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                                ...
                                <tr>
                                    ...
                                    <td className="vc-mds-tag-val-cell">
                                        <span className="vc-mds-tag-value">
                                            {data.fileInfo.dimensions || `${dynamicMedia.width} ├ù ${dynamicMedia.height}`}
                                        </span>
                        ...
                        <div>
                            ...
                            <div style={{ marginTop: "6px", display: "flex", justifyContent: "flex-end" }}>
                                <Button size={Button.Sizes.MIN} color={Button.Colors.PRIMARY} onClick={() => {
                                    ...
                                    showToast(t("╪¬┘à ┘å╪│╪« ╪º┘ä┘Ç prompt!", "Copied prompt!"), Toasts.Type.SUCCESS);
                                }}>Copy Prompt</Button>
                            </div>
                        </div>
                            ...
                            <div>
                                ...
                                <div style={{ marginTop: "6px", display: "flex", justifyContent: "flex-end" }}>
                                    <Button size={Button.Sizes.MIN} color={Button.Colors.PRIMARY} onClick={() => {
                                        ...
                                        showToast(t("╪¬┘à ┘å╪│╪« ╪º┘ä┘Ç negative prompt!", "Copied negative prompt!"), Toasts.Type.SUCCESS);
                        ...
                        <div className="vc-mds-sd-grid">
                            ...
                            {sdParams.seed && (
                                <div className="vc-mds-sd-badge" style={{ cursor: "pointer" }} onClick={() => {
                                    ...
                                    showToast(t("╪¬┘à ┘å╪│╪« ╪º┘ä┘Ç seed!", "Copied seed!"), Toasts.Type.SUCCESS);
                                }} title={t("╪º┘å┘é╪▒ ┘ä┘å╪│╪« Seed", "Click to copy Seed")}>
                    ...
                    <div>
                        ...
                        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "8px" }}>
                            <Button size={Button.Sizes.MIN} color={Button.Colors.PRIMARY} onClick={() => {
                                ...
                                showToast(t("╪¬┘à ┘å╪│╪« JSON ╪º┘ä╪«╪º┘à!", "Copied raw JSON!"), Toasts.Type.SUCCESS);
                    ...
                    <div className="vc-mds-main">
                        {loading ? (
                            <div className="vc-mds-loading-state">
                                ...
                                <span>Reading file structuresΓÇª</span>
                            </div>
                        ) : error ? (
                ...
                >
                    ...
                    >
                        ...
                        <button
                            ...
                            onMouseEnter={e => { e.currentTarget.style.opacity = "1"; }}
                            onMouseLeave={e => { e.currentTarget.style.opacity = "0.8"; }}
                            title={t("╪Ñ╪║┘ä╪º┘é ┘à┘ä╪í ╪º┘ä╪┤╪º╪┤╪⌐", "Close Fullscreen")}
                            type="button"
                        >
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
