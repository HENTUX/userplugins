...
const settings = definePluginSettings({
    uploader: {
        ...
        hidden: true,
        options: [
            { label: "Catbox (Ø­Ø¯ Ø£Ù‚ØµÙ‰ 200 Ù…ÙŠØºØ§Ø¨Ø§ÙŠØª)", value: "Catbox", default: true },
            { label: "Litterbox (Catbox Ù…Ø¤Ù‚ØªØŒ 1 Ø¬ÙŠØ¬Ø§Ø¨Ø§ÙŠØª)", value: "Litterbox" },
            { label: "GoFile", value: "GoFile" },
            { label: "FileFast (Ø³Ø±ÙŠØ¹)", value: "FileFast" },
            { label: "Ù…Ø®ØµØµ (Ù…ØªÙˆØ§ÙÙ‚ Ù…Ø¹ ShareX)", value: "Custom" },
        ]
    },
    autoSend: {
    ...
    renderImagePreviews: {
        type: OptionType.BOOLEAN,
        description: "Show inline image previews for Catbox/Litter/GoFile links even if AtlasXGOD does not embed them",
        hidden: true,
        default: true
    },
    litterboxTime: {
        ...
        hidden: true,
        options: [
            { label: "Ø³Ø§Ø¹Ø© ÙˆØ§Ø­Ø¯Ø©", value: "1h", default: true },
            { label: "12 Ø³Ø§Ø¹Ø©", value: "12h" },
            { label: "24 Ø³Ø§Ø¹Ø©", value: "24h" },
            { label: "72 Ø³Ø§Ø¹Ø©", value: "72h" },
        ]
    },
    ...
    customResponseType: {
        ...
        hidden: true,
        options: [
            { label: "Ù†Øµ", value: "Text", default: true },
...
async function confirmUploadIfNeeded(uploader: Uploader, payload: UploadPayload): Promise<boolean> {
    if (!settings.store.confirmBeforeUpload) {
        if (settings.store.showDestinationPreview) {
            ...
            showToast(`${t("Ø¬Ø§Ø±Ù Ø§Ù„Ø±ÙØ¹ Ø¥Ù„Ù‰", "Uploading to")} ${dest.host} (${dest.url})â€¦`, Toasts.Type.MESSAGE);
        }
        return true;
    }
    ...
    const body = (
        <div>
            <Forms.FormText className={Margins.bottom8}>
                {t("Ø£Ù†Øª Ø¹Ù„Ù‰ ÙˆØ´Ùƒ Ø±ÙØ¹", "You are about to upload")} <b>{payload.fileName}</b> ({(payload.fileSizeBytes / (1024 * 1024)).toFixed(2)} MiB) {t("Ø¥Ù„Ù‰", "to")} <b>{getUploaderDisplayName(uploader)}</b>.
            </Forms.FormText>
            {settings.store.showDestinationPreview && (
                <div className={Margins.top8}>
                    <Forms.FormText><b>{t("Ù…Ø¶ÙŠÙ Ø§Ù„ÙˆØ¬Ù‡Ø©:", "Destination Host:")}</b> {dest.host}</Forms.FormText>
                    <Forms.FormText><b>{t("Ø±Ø§Ø¨Ø· Ø§Ù„ÙˆØ¬Ù‡Ø©:", "Destination URL:")}</b> {dest.url}</Forms.FormText>
                    {dest.note && <Forms.FormText>{dest.note}</Forms.FormText>}
                    {uploader === "Custom" && (
                        <Forms.FormText className={Margins.top8}>
                            {t("Ù„Ù„Ø±Ø§ÙØ¹Ø§Øª Ø§Ù„Ù…Ø®ØµØµØ©ØŒ ØªØ£ÙƒØ¯ Ù…Ù† Ø§Ù„ÙˆØ«ÙˆÙ‚ Ø¨Ø§Ù„Ù€ endpoint ÙˆØ§Ù„ØªØ±ÙˆÙŠØ³Ø§Øª Ø§Ù„Ù…ÙØ¹Ø¯ÙŽÙ‘Ø©.", "For custom uploaders, make sure you trust the endpoint and configured headers.")}
                        </Forms.FormText>
                    )}
                </div>
            )}
    ...
    return await new Promise<boolean>(resolve => {
        Alerts.show({
            title: t("ØªØ£ÙƒÙŠØ¯ Ø§Ù„Ø±ÙØ¹", "Confirm Upload"),
            body,
            confirmText: t("Ø±ÙØ¹", "Upload"),
            cancelText: t("Ø¥Ù„ØºØ§Ø¡", "Cancel"),
...
function validateCustomSettings(): { ok: true; args: Record<string, string>; headers: Record<string, string>; urlPath: string[]; } | { ok: false; error: string; } {
    ...
    if (!requestUrl) return { ok: false, error: t("Ø§Ù„Ø±Ø§ÙØ¹ Ø§Ù„Ù…Ø®ØµØµ: Ø±Ø§Ø¨Ø· Ø§Ù„Ø·Ù„Ø¨ Ù…Ø·Ù„ÙˆØ¨.", "Custom uploader: Request URL is required.") };
    if (!isHttpUrl(requestUrl)) return { ok: false, error: t("Ø§Ù„Ø±Ø§ÙØ¹ Ø§Ù„Ù…Ø®ØµØµ: Ø±Ø§Ø¨Ø· Ø§Ù„Ø·Ù„Ø¨ ÙŠØ¬Ø¨ Ø£Ù† ÙŠÙƒÙˆÙ† Ø±Ø§Ø¨Ø· http(s) ØµØ­ÙŠØ­Ø§Ù‹.", "Custom uploader: Request URL must be a valid http(s) URL.") };

    const fileFormName = settings.store.customFileFormName?.trim() ?? "";
    if (!fileFormName) return { ok: false, error: t("Ø§Ù„Ø±Ø§ÙØ¹ Ø§Ù„Ù…Ø®ØµØµ: Ø§Ø³Ù… Ù†Ù…ÙˆØ°Ø¬ Ø§Ù„Ù…Ù„Ù Ù…Ø·Ù„ÙˆØ¨.", "Custom uploader: File form name is required.") };

    const responseType = settings.store.customResponseType as CustomResponseType;
    if (responseType !== "Text" && responseType !== "JSON")
        return { ok: false, error: t("Ø§Ù„Ø±Ø§ÙØ¹ Ø§Ù„Ù…Ø®ØµØµ: Ù†ÙˆØ¹ Ø§Ù„Ø§Ø³ØªØ¬Ø§Ø¨Ø© ÙŠØ¬Ø¨ Ø£Ù† ÙŠÙƒÙˆÙ† Ù†ØµØ§Ù‹ Ø£Ùˆ JSON.", "Custom uploader: Response type must be Text or JSON.") };
    ...
    if (responseType === "JSON" && urlPath.length === 0)
        return { ok: false, error: t("Ø§Ù„Ø±Ø§ÙØ¹ Ø§Ù„Ù…Ø®ØµØµ: Ù…Ø³Ø§Ø± Ø§Ù„Ø±Ø§Ø¨Ø· Ù…Ø·Ù„ÙˆØ¨ Ù„Ø§Ø³ØªØ¬Ø§Ø¨Ø§Øª JSON (Ù…Ø«Ø§Ù„: data.url).", "Custom uploader: URL path is required for JSON responses (e.g. data.url).") };
...
async function sendOrInsert(channelId: string, url: string) {
    if (settings.store.autoSend) {
        ...
        showToast(t("ØªÙ… Ø¥Ø±Ø³Ø§Ù„ Ø±Ø§Ø¨Ø· Ø§Ù„Ø±ÙØ¹!", "Upload link sent!"), Toasts.Type.SUCCESS);
    } else {
        insertTextIntoChatInputBox(`${url} `);
        await copyWithToast(url, t("ØªÙ… Ù†Ø³Ø® Ø±Ø§Ø¨Ø· Ø§Ù„Ø±ÙØ¹!", "Upload link copied!"));
    }
}
...
async function runUploadFlow(channelId: string) {
    ...
    try {
        ...
        toast.show(`${t("Ø¬Ø§Ø±Ù Ø§Ù„Ø±ÙØ¹ Ø¹Ø¨Ø±", "Uploading via")} ${getUploaderDisplayName(uploader)}â€¦`);
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                if (attempt > 1) toast.show(`${t("Ø¥Ø¹Ø§Ø¯Ø© Ù…Ø­Ø§ÙˆÙ„Ø© Ø§Ù„Ø±ÙØ¹", "Retrying upload")} (${attempt}/${maxAttempts})â€¦`);
                url = await uploadToService(uploader, payload);
                break;
            } catch (err) {
                if (attempt >= maxAttempts || !isRetryableNetworkError(err)) throw err;
                toast.show(`${t("Ø®Ø·Ø£ ÙÙŠ Ø§Ù„Ø´Ø¨ÙƒØ©Ø› Ø¥Ø¹Ø§Ø¯Ø© Ø§Ù„Ù…Ø­Ø§ÙˆÙ„Ø©", "Network error; retrying")} (${attempt + 1}/${maxAttempts})â€¦`, Toasts.Type.MESSAGE, 4000);
                await sleep(900 * attempt);
            }
        }

        if (typeof url !== "string" || !(url.startsWith("http://") || url.startsWith("https://")))
            throw new Error(t("Ø§ÙƒØªÙ…Ù„ Ø§Ù„Ø±ÙØ¹ Ù„ÙƒÙ† Ø§Ù„Ø±Ø§Ø¨Ø· Ø§Ù„Ù…ÙØ¹Ø§Ø¯ ØºÙŠØ± ØµØ§Ù„Ø­.", "Upload completed but returned URL is invalid."));

        const finalUrl = maybeWrapVideo(url, payload.fileName, payload.fileSizeBytes);
        await sendOrInsert(channelId, finalUrl);
        toast.show(t("Ø§ÙƒØªÙ…Ù„ Ø§Ù„Ø±ÙØ¹.", "Upload complete."), Toasts.Type.SUCCESS);
    } catch (err) {
        showToast(`${t("ÙØ´Ù„ Ø§Ù„Ø±ÙØ¹:", "Upload failed:")} ${humanError(err)}`, Toasts.Type.FAILURE);
    } finally {
        ...
        <Menu.MenuItem
            ...
            label={
                <div>
                    ...
                    <div>{t("Ø±ÙØ¹ Ù…Ù„Ù ÙƒØ¨ÙŠØ± (Enhanced)", "Upload Large File (Enhanced)")}</div>
                </div>
            }
...
function SettingsComponent() {
    ...
    const importShareXConfig = async () => {
        ...
        try {
            ...
            showToast(`${t("ÙØ´Ù„ ØªØ­Ù„ÙŠÙ„ JSON. ÙÙŠ Ù…Ù„Ù sxcu:", "Failed to parse JSON in sxcu file:")} ${humanError(err)}`, Toasts.Type.FAILURE);
            return;
        }
        ...
        showToast(t("ØªÙ… Ø§Ø³ØªÙŠØ±Ø§Ø¯ Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª Ø±Ø§ÙØ¹ ShareX.", "ShareX uploader settings imported."), Toasts.Type.SUCCESS);
    };

    return (
        <div>
            <Forms.FormTitle>{t("Ø§Ù„Ø£Ù…Ø§Ù†", "Security")}</Forms.FormTitle>
            <Forms.FormText className={Margins.bottom8}>
                {t("ÙŠØ±ÙØ¹ Ø§Ù„Ù…Ù„ÙØ§Øª Ø¥Ù„Ù‰ Ø®Ø§Ø¯Ù… Ø®Ø§Ø±Ø¬ÙŠ Ø«Ù… ÙŠØ´Ø§Ø±Ùƒ Ø§Ù„Ø±Ø§Ø¨Ø· Ø§Ù„Ù†Ø§ØªØ¬ ÙÙŠ Ø§Ù„Ø¯Ø±Ø¯Ø´Ø©.", "Uploads files to an external server then shares the resulting link in the chat.")}
            </Forms.FormText>

            <Forms.FormText className={Margins.bottom8}>
                {t("Ø§Ù„ÙˆØ¬Ù‡Ø© Ø§Ù„Ø­Ø§Ù„ÙŠØ©:", "Current destination:")} <b>{dest.host}</b> ({dest.url})
            </Forms.FormText>

            <Divider className={Margins.bottom16} />

            <Forms.FormTitle>{t("Ø¹Ø§Ù…", "General")}</Forms.FormTitle>
            <div className={Margins.bottom16}>
                <Select
                    options={[
                        ...
                        { label: t("Ù…Ø®ØµØµ (Ù…ØªÙˆØ§ÙÙ‚ Ù…Ø¹ ShareX)", "Custom (ShareX compatible)"), value: "Custom" },
                    ]}
                    serialize={v => v}
            ...
            <FormSwitch
                title={t("ØªØ£ÙƒÙŠØ¯ Ù‚Ø¨Ù„ Ø§Ù„Ø±ÙØ¹", "Confirm before upload")}
                description={t("Ø·Ù„Ø¨ ØªØ£ÙƒÙŠØ¯ Ù‚Ø¨Ù„ Ø§Ù„Ø¨Ø¯Ø¡ Ø¨Ø§Ù„Ø±ÙØ¹", "Request confirmation before starting the upload")}
                value={settings.store.confirmBeforeUpload}
                onChange={v => settings.store.confirmBeforeUpload = v}
            />

            <FormSwitch
                title={t("Ø¹Ø±Ø¶ Ù…Ø¹Ø§ÙŠÙ†Ø© Ø§Ù„ÙˆØ¬Ù‡Ø©", "Show destination preview")}
                description={t("Ø¹Ø±Ø¶ Ù…Ø¶ÙŠÙ Ø§Ù„ÙˆØ¬Ù‡Ø© ÙˆØ§Ù„Ø±Ø§Ø¨Ø· ÙÙŠ Ø­ÙˆØ§Ø± Ø§Ù„ØªØ£ÙƒÙŠØ¯", "Show destination host and URL in the confirmation dialog")}
                value={settings.store.showDestinationPreview}
                onChange={v => settings.store.showDestinationPreview = v}
            />

            <FormSwitch
                title={t("Ø¥Ø±Ø³Ø§Ù„ Ø§Ù„Ø±Ø§Ø¨Ø· ØªÙ„Ù‚Ø§Ø¦ÙŠØ§Ù‹", "Auto-send link")}
                description={t("Ø¥Ø°Ø§ Ø£ÙÙˆÙ‚ÙØŒ ÙŠÙÙ†Ø³Ø® Ø§Ù„Ø±Ø§Ø¨Ø· ÙˆÙŠÙØ¯Ø±Ø¬ ÙÙŠ ØµÙ†Ø¯ÙˆÙ‚ Ø§Ù„Ø¯Ø±Ø¯Ø´Ø©", "If disabled, the link is copied and inserted into the chat box")}
                value={settings.store.autoSend}
                onChange={v => settings.store.autoSend = v}
            />

            <FormSwitch
                title={t("ØªØºÙ„ÙŠÙ Ø¨Ù€ embeds.video (Ø§Ø®ØªÙŠØ§Ø±ÙŠ)", "Wrap with embeds.video (optional)")}
                description={t("ÙŠØºÙ„Ù‘Ù Ø±ÙˆØ§Ø¨Ø· Ø§Ù„ÙÙŠØ¯ÙŠÙˆ Ø§Ù„ÙƒØ¨ÙŠØ±Ø© Ø¨Ù€ embeds.video (ÙŠØ¶ÙŠÙ Ø¥Ø¹Ø§Ø¯Ø© ØªÙˆØ¬ÙŠÙ‡ Ù…Ù† Ø·Ø±Ù Ø«Ø§Ù„Ø«)", "Wraps large video links with embeds.video (adds a third-party redirect)")}
            ...
            {uploader === "GoFile" && (
                <>
                    ...
                    <TextInput
                        label={t("Ø§Ù„Ø±Ù…Ø² Ø§Ù„Ù…Ù…ÙŠØ² (Ø§Ø®ØªÙŠØ§Ø±ÙŠØŒ ÙŠÙØ­ÙØ¸ Ø¨Ø´ÙƒÙ„ Ø®Ø§Øµ)", "Token (optional, stored privately)")}
                        type="password"
                        value={settings.store.gofileToken ?? ""}
                        placeholder={t("Ø±Ù…Ø² GoFile", "GoFile token")}
            ...
            {uploader === "Catbox" && (
                <>
                    ...
                    <TextInput
                        label={t("Ù‡Ø§Ø´ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù… (Ø§Ø®ØªÙŠØ§Ø±ÙŠØŒ ÙŠÙØ­ÙØ¸ Ø¨Ø´ÙƒÙ„ Ø®Ø§Øµ)", "User hash (optional, stored privately)")}
                        type="password"
                        value={settings.store.catboxUserHash ?? ""}
                        placeholder={t("Ù‡Ø§Ø´ Ù…Ø³ØªØ®Ø¯Ù… Catbox", "Catbox user hash")}
            ...
            {uploader === "FileFast" && (
                <>
                    ...
                    <Forms.FormText className={Margins.bottom8}>
                        {t("Ø±ÙØ¹ Ù…Ù„ÙØ§Øª Ø³Ø±ÙŠØ¹. Ø§Ù„Ù…Ù„ÙØ§Øª Ù„Ø§ ØªÙ†ØªÙ‡ÙŠ ØµÙ„Ø§Ø­ÙŠØªÙ‡Ø§.", "Fast file uploads. Files do not expire.")}
                    </Forms.FormText>
                    <TextInput
                        label={t("Ø§Ù„Ø±Ù…Ø² Ø§Ù„Ù…Ù…ÙŠØ² (Ø§Ø®ØªÙŠØ§Ø±ÙŠØŒ ÙŠÙØ­ÙØ¸ Ø¨Ø´ÙƒÙ„ Ø®Ø§Øµ)", "Token (optional, stored privately)")}
                        type="password"
                        value={settings.store.filefastToken ?? ""}
                        placeholder={t("Ø±Ù…Ø² FileFast", "FileFast token")}
            ...
            {uploader === "Litterbox" && (
                <>
                    ...
                    <div className={Margins.bottom16}>
                        <Select
                            options={[
                                { label: t("Ø³Ø§Ø¹Ø© ÙˆØ§Ø­Ø¯Ø©", "1 hour"), value: "1h" },
                                { label: t("12 Ø³Ø§Ø¹Ø©", "12 hours"), value: "12h" },
                                { label: t("24 Ø³Ø§Ø¹Ø©", "24 hours"), value: "24h" },
                                { label: t("72 Ø³Ø§Ø¹Ø©", "72 hours"), value: "72h" },
                            ]}
                            serialize={v => v}
            ...
            {uploader === "Custom" && (
                <>
                    <Divider className={Margins.bottom16} />
                    <Forms.FormTitle>{t("Ù…Ø®ØµØµ (ShareX)", "Custom (ShareX)")}</Forms.FormTitle>
                    <div className={Margins.bottom16}>
                        <Button onClick={importShareXConfig}>{t("Ø§Ø³ØªÙŠØ±Ø§Ø¯ Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª ShareX", "Import ShareX config")}</Button>
                    </div>

                    <TextInput
                        label={t("Ø§Ù„Ø§Ø³Ù… (Ù„Ù„Ø¹Ø±Ø¶ ÙÙ‚Ø·)", "Name (display only)")}
                        value={settings.store.customName}
                        onChange={v => settings.store.customName = v}
                        placeholder={t("Ø±Ø§ÙØ¹ÙŠ Ø§Ù„Ù…Ø®ØµØµ", "My custom uploader")}
                    />

                    <TextInput
                        label={t("Ø±Ø§Ø¨Ø· Ø§Ù„Ø·Ù„Ø¨", "Request URL")}
                    ...
                    <TextInput
                        label={t("Ø§Ø³Ù… Ù†Ù…ÙˆØ°Ø¬ Ø§Ù„Ù…Ù„Ù", "File form name")}
                    ...
                    <TextInput
                        label={t("Ù…Ø³Ø§Ø± Ø§Ù„Ø±Ø§Ø¨Ø· (Ù†Ù‚Ø·Ø© JSONØŒ Ø£Ùˆ ÙŠÙØªØ¬Ø§Ù‡Ù„ Ù„Ù„Ù†Øµ)", "URL path (JSON dot-path, or ignored for Text)")}
                    ...
                    <div className={Margins.bottom16}>
                        <Forms.FormText>{t("Ù…Ø¹Ø·ÙŠØ§Øª JSON (ØªÙØ­ÙØ¸ Ø¨Ø´ÙƒÙ„ Ø®Ø§Øµ)", "JSON Arguments (stored privately)")}</Forms.FormText>
                        <TextArea
                    ...
                    <div className={Margins.bottom16}>
                        <Forms.FormText>{t("ØªØ±ÙˆÙŠØ³Ø§Øª JSON (ØªÙØ­ÙØ¸ Ø¨Ø´ÙƒÙ„ Ø®Ø§Øµ)", "JSON Headers (stored privately)")}</Forms.FormText>
                        <TextArea
                    ...
                    <Button
                        onClick={() => {
                            ...
                            if (result.ok) showToast(t("Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª Ø§Ù„Ø±Ø§ÙØ¹ Ø§Ù„Ù…Ø®ØµØµ ØªØ¨Ø¯Ùˆ ØµØ­ÙŠØ­Ø©.", "Custom uploader settings look correct."), Toasts.Type.SUCCESS);
                            else showToast(result.error, Toasts.Type.FAILURE);
                        }}
                    >
                        {t("Ø§Ù„ØªØ­Ù‚Ù‚ Ù…Ù† Ø§Ù„Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª Ø§Ù„Ù…Ø®ØµØµØ©", "Validate custom settings")}
                    </Button>
                </>
            )}
        </div>
...
export default definePlugin({
    ...
    description: "Bypass AtlasXGOD's upload limit by uploading files to an external server and sharing the link in chat â€” this version is faster and does not use DOM manipulation.",
    tags: ["Utility", "Chat"],
