...
function shouldSilence(channelId: string, channel: any): boolean {
    if (!channel) {
        debug(`unknown channel ${channelId} ΓÇö passing through`);
        return false;
    }
...
export default definePlugin({
    ...
    description: "Start DM and group calls without ringing the other members ΓÇö they can still see and join the call, they just don't get the incoming-call notification.",
    ...
    start() {
        ...
        if (originalRing) {
            debug("start() called while already patched ΓÇö ignoring");
            return;
        }
        ...
        try {
            ...
            logger.error("could not find ring/stopRinging module ΓÇö plugin inactive", e);
            return;
        }

        if (typeof ring !== "function") {
            logger.error("could not find ring/stopRinging module ΓÇö plugin inactive");
            return;
        }
        ...
        CallActions.ring = function (this: unknown, channelId: string, ...rest: any[]) {
            ...
            if (shouldSilence(channelId, channel)) {
                ...
                debug(`ΓåÆ silencing (skipping ring) for ${channelId}`);
                return;
            }

            debug(`ΓåÆ ringing normally: ${channelId}`);
