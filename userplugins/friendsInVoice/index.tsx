...
import { ModalContent, ModalHeader, ModalRoot, openModal } from "@utils/esharqModals";
 ...
 * guild ΓÇö that is why a friend sitting in a server you never opened this session
...
function describe(userId: string, channelId: string | null): Entry | null {
    ...
    return {
        ...
        channelName: isCall ? t("┘à┘â╪º┘ä┘à╪⌐", "Call") : channel.name,
        placeName: isCall
            ? t("╪º┘ä╪▒╪│╪º╪ª┘ä ╪º┘ä╪«╪º╪╡╪⌐", "Direct messages")
        ...
        <div className={cl("row")}>
            ...
            <div className={cl("info")}>
                <Text variant="text-sm/semibold" className={cl("name")}>
                    {entry.isWatched ? `Γÿà ${entry.name}` : entry.name}
                </Text>
                <Text variant="text-xs/normal" className={cl("where")}>
                    {entry.channelId
                        ? (entry.placeName ? `${entry.placeName} ΓÇö ${entry.channelName}` : entry.channelName)
                        : t("┘ä┘è╪│ ┘ü┘è ╪▒┘ê┘à ╪╡┘ê╪¬┘è", "Not in a voice channel")}
                    {entry.isHidden && <span className={cl("state")}>≡ƒöÆ {t("┘à╪«┘ü┘è", "Hidden")}</span>}
                    {entry.isLocked && <span className={cl("state")}>≡ƒöÆ {t("┘à┘é┘ü┘ä", "Locked")}</span>}
                </Text>
            </div>
            {entry.channelId && (
                <button
                ...
                >
                    {entry.canJoin ? t("╪º┘å╪╢┘à╪º┘à", "Join") : t("┘ä╪º ╪╡┘ä╪º╪¡┘è╪⌐", "No access")}
                </button>
            )}
        </div>
    );
...
function FriendsInVoicePanel({ transitionState }: { transitionState: any; }) {
    ...
    return (
        <ModalRoot transitionState={transitionState} size={ModalSize.SMALL}>
            <ModalHeader>
                <Text variant="heading-lg/semibold">{t("╪º┘ä╪ú╪╡╪»┘é╪º╪í ┘ü┘è ╪º┘ä╪╡┘ê╪¬", "Friends in voice")}</Text>
            </ModalHeader>
            <ModalContent>
                <div className={cl("panel")} dir={isArabicMode() ? "rtl" : "ltr"}>
                    {entries.length === 0 ? (
                        <div className={cl("empty")}>
                            {t(
                                "┘ä╪º ╪ú╪¡╪» ╪º┘ä╪ó┘å. ┘â┘ä┘è┘â ┘è┘à┘è┘å ╪╣┘ä┘ë ╪┤╪«╪╡ ΓåÉ ┬½┘à╪▒╪º┘é╪¿╪⌐ ┘ü┘è ╪º┘ä╪╡┘ê╪¬┬╗ ┘ä┘è╪╕┘ç╪▒ ┘ç┘å╪º ╪»╪º╪ª┘à╪º┘ï.",
                                "Nobody right now. Right-click someone ΓåÆ \"Watch in voice\" to always track them here."
                            )}
                        </div>
                    ) : (
...
function FriendsInVoiceButton() {
    ...
    return (
        <HeaderBarButton
            icon={PeopleIcon}
            tooltip={inVoice > 0
                ? t(`╪º┘ä╪ú╪╡╪»┘é╪º╪í ┘ü┘è ╪º┘ä╪╡┘ê╪¬ (${inVoice})`, `Friends in voice (${inVoice})`)
                : t("╪º┘ä╪ú╪╡╪»┘é╪º╪í ┘ü┘è ╪º┘ä╪╡┘ê╪¬", "Friends in voice")}
            selected={inVoice > 0}
            aria-label={t("╪º┘ä╪ú╪╡╪»┘é╪º╪í ┘ü┘è ╪º┘ä╪╡┘ê╪¬", "Friends in voice")}
            onClick={() => {
...
const userContextPatch = (children: any[], props: any) => {
    ...
    group.push(
        <Menu.MenuCheckboxItem
            id="vc-fiv-watch"
            key="vc-fiv-watch"
            label={t("┘à╪▒╪º┘é╪¿╪⌐ ┘ü┘è ╪º┘ä╪╡┘ê╪¬", "Watch in voice")}
