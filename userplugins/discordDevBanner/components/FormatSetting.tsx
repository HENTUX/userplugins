...
export function FormatSetting(setValue) {
    ...
    const [state, setState] = useState(format ?? "{buildChannel} {buildNumber} ({buildHash}) | {AtlasXGODName} {AtlasXGODVersion} ({AtlasXGODHash})");
    ...
    return (
        <section>
            <Paragraph className={"vc-discord-dev-banner-text"}>
                The format for the Dev Banner. You can use the following variables:
                {"\n\n"}
            ...
            <textarea
                ...
                placeholder="{buildChannel} {buildNumber} ({buildHash}) | {AtlasXGODName} {AtlasXGODVersion} ({AtlasXGODHash})"
                rows={1}
                ref={el => {
                    if (el) {
