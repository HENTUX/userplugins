...
import { ChromiumIcon, ClientIcon, DevBannerIcon, DiscordIcon, ElectronIcon, AtlasXGODIcon, names, settings } from ".";
...
export function makeDevBanner(state?: string): string | JSX.Element {
    ...
    const format = settings.store.format ?? "{devbannerIcon} {buildChannel} {buildNumber} ({buildHash}) | {AtlasXGODIcon} {AtlasXGODName} {AtlasXGODVersion} ({AtlasXGODHash})";
    ...
    const replaced = baseFormat
        ...
        .replace(/{AtlasXGODVersion}/g, VERSION)
        .replace(/{AtlasXGODHash}/g, gitHashShort)
        .replace(/{AtlasXGODPlatform}/g, getVersionInfo(false))
    ...
    const parts = replaced.split(/({.*?}|__NEWLINE__)/).filter(Boolean).map((part, i) => {
        switch (part) {
            ...
            case "{AtlasXGODIcon}":
                return <span key={`icon-AtlasXGOD-${i}`} className="vc-discord-dev-banner-icons"><AtlasXGODIcon /></span>;
            case "{electronIcon}":
