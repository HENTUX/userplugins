...
// Scan cache ΓÇö avoids rescanning if done recently
        ...
        setTimeout(() => {
            try {
                ...
                for (const guildId in guilds) {
                    ...
                    // Collect ALL items from ALL keys ΓÇö don't assume the key names
...
function VoiceSearchModal({ rootProps, channels }: { rootProps: any; channels: VoiceChannel[] | null; }) {
    ...
    async function join(ch: VoiceChannel) {
        ...
        try {
            ...
            // Petit d├⌐lai pour l'effet visuel avant de fermer
        ...
        <ModalRoot {...rootProps} size="medium">
            <ModalHeader separator={false}>
                <Forms.FormTitle tag="h4" style={{ margin: 0, display: "flex", alignItems: "center", gap: 8, color: "#ffffff" }}>
                    <SearchIcon width={16} height={16} /> {t("╪º┘ä┘é┘å┘ê╪º╪¬ ╪º┘ä╪╡┘ê╪¬┘è╪⌐", "Voice Channels")}
            ...
            <ModalContent className="vcs-content">
                {channels === null ? (
                    <div className="vcs-loading">
                        <SpinnerIcon />
                        <span>{t("╪¼╪º╪▒┘ì ╪¬╪¡┘à┘è┘ä ╪º┘ä┘é┘å┘ê╪º╪¬...", "Loading channels...")}</span>
                    </div>
                ) : (
                    <>
                        <div className="vcs-search-bar">
                            ...
                            <input
                                ...
                                placeholder={t("┘é┘å╪º╪⌐ ╪ú┘ê ╪│┘è╪▒┘ü╪▒...", "Channel or server...")}
                                value={query}
                        ...
                        <div className="vcs-channel-list">
                            {displayList!.length === 0 && (
                                <div className="vcs-empty">{query ? t("┘ä╪º ╪¬┘ê╪¼╪» ┘é┘å╪º╪⌐", "No channel found") : t("┘ä╪º ┘é┘å┘ê╪º╪¬ ╪╡┘ê╪¬┘è╪⌐", "No voice channels")}</div>
                            )}
                            {displayList!.slice(0, 150).map(ch => (
                                <div key={ch.channelId}
                                    ...
                                    title={ch.canAccess ? undefined : t("┘ä╪º ╪╡┘ä╪º╪¡┘è╪⌐ ┘ä┘ä╪º┘å╪╢┘à╪º┘à ╪Ñ┘ä┘ë ┘ç╪░┘ç ╪º┘ä┘é┘å╪º╪⌐", "No permission to join this channel")}
                                >
                                    <span className="vcs-icon">
                                        {ch.canAccess
                                            ? (ch.channelType === 13 ? <StageIcon /> : <VoiceIcon />)
                                            : <span style={{ opacity: 0.5, fontSize: 13 }}>≡ƒöÆ</span>
                                        }
                                    </span>
                                    ...
                                    {joiningId === ch.channelId
                                        ? <span className="vcs-joining-label">{t("╪¼╪º╪▒┘ì ╪º┘ä╪º┘å╪╢┘à╪º┘à...", "Joining...")}</span>
                                        : ch.canAccess
                                            ? <button className="vcs-join-btn" onClick={e => { e.stopPropagation(); join(ch); }}>{t("╪º┘å╪╢┘à╪º┘à", "Join")}</button>
                                            : <span className="vcs-locked-label">{t("╪«╪º╪╡┘æ╪⌐", "Private")}</span>
                                    }
                                </div>
                            ))}
                            {displayList!.length > 80 && !query && (
                                <div className="vcs-empty" style={{ fontSize: 11, opacity: 0.5 }}>
                                    {displayList!.length - 80} {t("┘é┘å╪º╪⌐ ╪Ñ╪╢╪º┘ü┘è╪⌐ ΓÇö ╪º╪│╪¬╪«╪»┘à ╪º┘ä╪¿╪¡╪½", "more channels ΓÇö use search")}
                                </div>
                            )}
                        </div>
                    </>
...
function VCSHeaderButton() {
    return (
        <HeaderBarButton
            icon={SearchIcon}
            tooltip={t("╪¿╪¡╪½ ╪╣┘å ┘é┘å╪º╪⌐ ╪╡┘ê╪¬┘è╪⌐", "Search voice channel")}
