...
async function applyTarget(guildId: string, userId: string, nick: string | null) {
    ...
    try {
        ...
        if (settings.store.showToast) {
            showToast(t("╪ú╪╣╪º╪»╪¬ AutoNickname ╪º┘ä╪º╪│┘à ╪º┘ä┘à╪│╪¬╪╣╪º╪▒.", "AutoNickname restored a nickname."), Toasts.Type.SUCCESS);
        }
    } catch {
        if (settings.store.showToast) {
            showToast(t("╪¬╪╣╪░┘æ╪▒ ╪╣┘ä┘ë AutoNickname ╪Ñ╪╣╪º╪»╪⌐ ╪º┘ä╪º╪│┘à ╪º┘ä┘à╪│╪¬╪╣╪º╪▒.", "AutoNickname failed to restore a nickname."), Toasts.Type.FAILURE);
        }
    } finally {
...
function AutoNicknameModal({ modalProps, guildId, user }: { modalProps: ModalProps; guildId: string; user: User; }) {
    ...
    return (
        <ModalRoot {...modalProps} size={ModalSize.SMALL}>
            <ModalHeader>
                <Text variant="heading-lg/semibold" tag="h1">{t("╪º┘ä┘ä┘é╪¿ ╪º┘ä╪¬┘ä┘é╪º╪ª┘è", "Auto Nickname")}</Text>
            ...
            <ModalContent>
                ...
                <TextInput
                    value={nick}
                    onChange={setNick}
                    placeholder={t("╪º┘ä┘ä┘é╪¿", "Nickname")}
                    autoFocus
                />
            </ModalContent>
            <ModalFooter>
...
const UserContextMenuPatch: NavContextMenuPatchCallback = (children, props: UserContextProps) => {
    ...
    children.push(
        <Menu.MenuItem
            id="vc-autonickname"
            label={t("╪º┘ä┘ä┘é╪¿ ╪º┘ä╪¬┘ä┘é╪º╪ª┘è", "Auto Nickname")}
        >
            <Menu.MenuItem
            ...
            {savedTarget && (
                <Menu.MenuItem
                    ...
                    label={t("╪¬╪╣╪╖┘è┘ä ╪º┘ä┘ä┘é╪¿ ╪º┘ä╪¬┘ä┘é╪º╪ª┘è", "Disable Auto Nickname")}
                    color="danger"
