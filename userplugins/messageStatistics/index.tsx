    ...
    return (
        <div style={{ color: "var(--text-normal)" }}>
            <div style={{ marginBottom: 20, background: "var(--background-secondary)", padding: 15, borderRadius: 8 }}>
                <h3 style={{ marginTop: 0, color: "var(--header-primary)" }}>≡ƒÄ» Daily goal</h3>
            ...
            <h3 style={{ color: "var(--header-primary)", marginBottom: 10 }}>≡ƒôè Stats</h3>
...
export default definePlugin({
    ...
    commands: [
        {
            name: "mystats",
            description: t("╪º╪╣╪▒╪╢ ╪Ñ╪¡╪╡╪º╪í╪º╪¬ ╪▒╪│╪º╪ª┘ä┘â ┘ç┘å╪º ┘ü┘è ╪º┘ä╪»╪▒╪»╪┤╪⌐", "Show your message stats here in chat"),
            options: [],
            execute: async (_, ctx) => {
                ...
                sendBotMessage(ctx.channel.id, {
                    content: `≡ƒôè **Your messages** ΓÇö today **${stats.days[dayKey(now)] ?? 0}** ┬╖ week **${since(7 * DAY)}** ┬╖ month **${since(30 * DAY)}** ┬╖ year **${since(365 * DAY)}** ┬╖ all-time **${stats.total}**`
                });
            }
        }
    ],
