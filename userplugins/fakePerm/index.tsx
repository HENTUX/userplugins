...
// ΓöÇΓöÇΓöÇ Common Styles ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
function RenameModal({ rootProps, user, guildId }: { rootProps: any; user: any; guildId: string | null; }) {
    ...
    function applyNick() {
        ...
        toast(t(`╪¬┘à ╪¬╪║┘è┘è╪▒ ╪º┘ä╪º╪│┘à ╪º┘ä┘à╪│╪¬╪╣╪º╪▒ ΓåÉ ${trimmed || "(╪Ñ╪╣╪º╪»╪⌐ ╪¬╪╣┘è┘è┘å)"}`, `Nickname changed ΓåÆ ${trimmed || "(reset)"}`));
        rootProps.onClose();
    }
    return (
        <ModalRoot {...rootProps} size="small">
            <ModalHeader separator={false}><h2 style={{ ...modalTitle, flex: 1 }}>{t("╪¬╪║┘è┘è╪▒ ╪º┘ä╪º╪│┘à ╪º┘ä┘à╪│╪¬╪╣╪º╪▒", "Change Nickname")}</h2><ModalCloseButton onClick={rootProps.onClose} /></ModalHeader>
            <ModalContent style={{ padding: "0 16px 20px" }}>
                <div style={sectionLabel()}>{t("╪º┘ä╪º╪│┘à ╪º┘ä┘à╪│╪¬╪╣╪º╪▒", "Nickname")}</div>
            ...
            <ModalFooter>
                <div style={{ display: "flex", gap: "12px", width: "100%", padding: "16px" }}>
                    <button onClick={rootProps.onClose} style={footerBtn("#4e5058") as any}>{t("╪Ñ┘ä╪║╪º╪í", "Cancel")}</button>
                    <button onClick={applyNick} style={footerBtn("#5865f2") as any}>{t("╪¬╪╖╪¿┘è┘é", "Apply")}</button>
                </div>
            </ModalFooter>
        </ModalRoot>
    );
        ...
        <ModalRoot {...rootProps} size="small">
            <ModalHeader separator={false}>
                <h2 style={{ fontFamily: "var(--font-display)", fontSize: "20px", fontWeight: 700, color: "#ffffff", margin: 0, flex: 1 }}>
                    {t(`╪╖╪▒╪» ${username} ┘à┘å ╪º┘ä╪«╪º╪»┘à`, `Kick ${username} from server`)}
                </h2>
            ...
            <ModalContent style={{ padding: "8px 16px 20px" }}>
                <p style={{ fontFamily: "var(--font-primary)", fontSize: "14px", color: "#ffffff", lineHeight: "20px", marginBottom: "16px", marginTop: "4px" }}>
                    {t(`┘ç┘ä ╪ú┘å╪¬ ┘à╪¬╪ú┘â╪» ┘à┘å ╪╖╪▒╪» @${tag} ┘à┘å ╪º┘ä╪«╪º╪»┘à╪ƒ ╪│┘è╪¬┘à┘â┘æ┘å ┘à┘å ╪º┘ä╪╣┘ê╪»╪⌐ ╪¿╪»╪╣┘ê╪⌐ ╪¼╪»┘è╪»╪⌐.`, `Are you sure you want to kick @${tag} from the server? They will be able to return with a new invitation.`)}
                </p>
                <div style={{ fontFamily: "var(--font-primary)", fontSize: "14px", fontWeight: 600, color: "#ffffff", marginBottom: "8px" }}>{t("╪│╪¿╪¿ ╪º┘ä╪╖╪▒╪»", "Reason for kick")}</div>
                <textarea
                    value={reason}
            ...
            <ModalFooter>
                <div style={{ display: "flex", gap: "8px", width: "100%", padding: "16px" }}>
                    <button onClick={rootProps.onClose} style={footerBtn("#4e5058") as any}>{t("╪Ñ┘ä╪║╪º╪í", "Cancel")}</button>
                    <button onClick={() => { kickedUsers.add(user.id); disconnectedUsers.add(user.id); notifyMemberListChange(); toast(t(`╪¬┘à ╪╖╪▒╪» @${tag} (┘à╪¡┘ä┘è)`, `@${tag} kicked (local)`)); rootProps.onClose(); }}
...
const BAN_REASONS = [
    { label: t("╪¡╪│╪º╪¿ ┘à╪┤╪¿┘ê┘ç ╪ú┘ê ╪▒╪│╪º╪ª┘ä ┘à╪▓╪╣╪¼╪⌐", "Suspicious account or spam"), value: "spam" },
    { label: t("╪¡╪│╪º╪¿ ┘à┘Å╪«╪¬╪▒┘Ä┘é ╪ú┘ê ┘à┘Å╪«╪¬╪╖┘Ä┘ü", "Compromised or hacked account"), value: "compromised" },
    { label: t("┘à╪«╪º┘ä┘ü╪⌐ ┘é┘ê╪º╪╣╪» ╪º┘ä╪«╪º╪»┘à", "Non-respect of server rules"), value: "rules" },
    { label: t("╪ú╪«╪▒┘ë", "Other"), value: "other" },
];
const DELETE_OPTIONS = [
    { label: t("╪╣╪»┘à ╪¡╪░┘ü ╪ú┘è ╪┤┘è╪í", "Don't delete anything"), value: "0" },
    { label: t("╪ó╪«╪▒ ╪│╪º╪╣╪⌐", "Last hour"), value: "3600" },
    { label: t("╪ó╪«╪▒ 24 ╪│╪º╪╣╪⌐", "Last 24 hours"), value: "86400" },
    { label: t("╪ó╪«╪▒ 7 ╪ú┘è╪º┘à", "Last 7 days"), value: "604800" },
];
...
function BanModal({ rootProps, user }: { rootProps: any; user: any; }) {
    ...
    return (
        <ModalRoot {...rootProps} size="small">
            <ModalHeader separator={false}><h2 style={{ ...modalTitle, flex: 1 }}>{t(`╪¡╪╕╪▒ @${user.username}╪ƒ`, `Ban @${user.username}?`)}</h2><ModalCloseButton onClick={rootProps.onClose} /></ModalHeader>
            <ModalContent style={{ padding: "0 16px 20px" }}>
                <div style={sectionLabel()}>{t("╪º┘ä╪│╪¿╪¿", "Reason")}</div>
                ...
                <div style={sectionLabel()}>{t("╪¡╪░┘ü ╪º┘ä╪▒╪│╪º╪ª┘ä", "Delete messages")}</div>
            ...
            <ModalFooter>
                <div style={{ display: "flex", gap: "12px", width: "100%", padding: "16px", justifyContent: "flex-end" }}>
                    <Button look={Button.Looks.LINK} color={Button.Colors.PRIMARY} onClick={rootProps.onClose}>{t("╪Ñ┘ä╪║╪º╪í", "Cancel")}</Button>
                    <Button look={Button.Looks.FILLED} color={Button.Colors.RED} onClick={() => { if (!reason) return toast(t("╪º╪«╪¬╪▒ ╪│╪¿╪¿╪º┘ï", "Select a reason")); bannedUsers.add(user.id); kickedUsers.add(user.id); disconnectedUsers.add(user.id); notifyMemberListChange(); toast(t(`╪¬┘à ╪¡╪╕╪▒ @${user.username} (┘à╪¡┘ä┘è)`, `@${user.username} banned (local)`)); rootProps.onClose(); }}>{t("╪¡╪╕╪▒", "Ban")}</Button>
                </div>
            </ModalFooter>
        </ModalRoot>
    );
...
const TIMEOUT_DURATIONS = [
    { label: t("60 ╪½╪º┘å┘è╪⌐", "60 sec"), seconds: 60 }, { label: t("5 ╪»┘é╪º╪ª┘é", "5 min"), seconds: 300 },
    { label: t("10 ╪»┘é╪º╪ª┘é", "10 min"), seconds: 600 }, { label: t("╪│╪º╪╣╪⌐", "1 hour"), seconds: 3600 },
    { label: t("┘è┘ê┘à", "1 day"), seconds: 86400 }, { label: t("╪ú╪│╪¿┘ê╪╣", "1 week"), seconds: 604800 },
];
...
function TimeoutModal({ rootProps, user }: { rootProps: any; user: any; }) {
    ...
    return (
        <ModalRoot {...rootProps} size="small">
            <ModalHeader separator={false}>
                <h2 style={{ fontFamily: "var(--font-display)", fontSize: "20px", fontWeight: 700, color: "#ffffff", margin: 0, flex: 1 }}>
                    {t(`┘à┘ç┘ä╪⌐ ${username}`, `Timeout ${username}`)}
                </h2>
                <ModalCloseButton onClick={rootProps.onClose} />
            </ModalHeader>
            <ModalContent style={{ padding: "8px 16px 20px" }}>
                <p style={{ fontFamily: "var(--font-primary)", fontSize: "14px", color: "#ffffff", lineHeight: "20px", marginBottom: "20px", marginTop: "4px" }}>
                    {t("┘ä╪º ┘è╪│╪¬╪╖┘è╪╣ ╪º┘ä╪ú╪╣╪╢╪º╪í ╪º┘ä┘à┘ü╪▒┘ê╪╢╪⌐ ╪╣┘ä┘è┘ç┘à ┘à┘ç┘ä╪⌐ ┘à╪ñ┘é╪¬╪⌐ ╪Ñ╪▒╪│╪º┘ä ╪º┘ä╪▒╪│╪º╪ª┘ä ╪ú┘ê ╪º┘ä╪¬┘ü╪º╪╣┘ä ┘ü┘è ╪º┘ä┘é┘å┘ê╪º╪¬ ╪º┘ä┘å╪╡┘è╪⌐╪î ┘ê┘ä╪º ┘è┘Å╪│┘à╪¡ ┘ä┘ç┘à ╪¿╪º┘ä╪º┘å╪╢┘à╪º┘à ╪Ñ┘ä┘ë ╪º┘ä┘é┘å┘ê╪º╪¬ ╪º┘ä╪╡┘ê╪¬┘è╪⌐ ╪ú┘ê ┘é┘å┘ê╪º╪¬ ╪º┘ä┘à╪ñ╪¬┘à╪▒╪º╪¬.", "Temporarily timed out members cannot send messages or react in text channels. They are also not allowed to join voice or conference channels.")}{" "}
                    <span style={{ color: "#00a8fc", cursor: "pointer" }}>{t("╪º╪╣╪▒┘ü ╪º┘ä┘à╪▓┘è╪»", "Learn more")}</span>
                </p>
                <div style={{ fontFamily: "var(--font-primary)", fontSize: "14px", fontWeight: 600, color: "#ffffff", marginBottom: "8px" }}>{t("╪º┘ä┘à╪»┘æ╪⌐", "Duration")}</div>
                ...
                <div style={{ fontFamily: "var(--font-primary)", fontSize: "14px", fontWeight: 600, color: "#ffffff", marginBottom: "8px" }}>{t("╪º┘ä╪│╪¿╪¿", "Reason")}</div>
                <textarea
                    value={reason}
                    onChange={e => setReason(e.target.value)}
                    placeholder={t("╪ú╪»╪«┘ä ╪│╪¿╪¿╪º┘ï. ╪│┘è╪╕┘ç╪▒ ┘ü┘é╪╖ ┘ü┘è ╪│╪¼┘ä┘æ╪º╪¬ ╪º┘ä╪«╪º╪»┘à ┘ê┘ä┘å ┘è╪¬┘à┘â┘æ┘å ┘ç╪░╪º ╪º┘ä╪╣╪╢┘ê ┘à┘å ╪▒╪ñ┘è╪¬┘ç.", "Enter a reason. It will only be visible in server logs and this member won't be able to see it.")}
            ...
            <ModalFooter>
                <div style={{ display: "flex", gap: "8px", width: "100%", padding: "16px" }}>
                    ...
                    <button onClick={() => {
                        ...
                        notifyMemberListChange();
                        toast(t(`╪¬┘à ┘ü╪▒╪╢ ┘à┘ç┘ä╪⌐ ╪╣┘ä┘ë @${tag} ┘ä┘à╪»┘æ╪⌐ ${d.label} (┘à╪¡┘ä┘è)`, `@${tag} timed out for ${d.label} (local)`));
        ...
        <ModalRoot {...rootProps} size="small">
            <ModalContent style={{ padding: "8px 0 0", background: "var(--background-floating, #18191c)", borderRadius: 8, minWidth: 220 }}>
                <div style={{ padding: "4px 8px" }}>
                    <input autoFocus placeholder={t("╪º┘ä╪▒╪¬╪¿╪⌐", "Role")} value={search} onChange={e => setSearch(e.target.value)} style={{ width: "100%", background: "transparent", border: "1px solid var(--brand-experiment, #5865f2)", borderRadius: 4, outline: "none", color: "var(--text-normal, #dcddde)", fontSize: 14, padding: "4px 8px", boxSizing: "border-box" }} />
                </div>
                <div style={{ maxHeight: 300, overflowY: "auto", scrollbarWidth: "none", padding: "4px 0" }}>
                    {filtered.map(role => {
                        ...
                        return (
                            <div key={role.id} onClick={() => { toast(t(`╪º┘ä╪▒╪¬╪¿╪⌐ ${role.name} ΓÇö ┘à╪¡╪º┘â╪º╪⌐`, `Role ${role.name} ΓÇö simulation`)); rootProps.onClose(); }} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 12px", cursor: "pointer", opacity: memberRoleIds.includes(role.id) ? 0.5 : 1 }} onMouseEnter={e => (e.currentTarget.style.background = "var(--background-modifier-hover)")} onMouseLeave={e => (e.currentTarget.style.background = "transparent")}>
...
// ΓöÇΓöÇΓöÇ Context menu patches ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    try {
        ...
        children.splice(-1, 0, (
            <Menu.MenuGroup key="fp-msg-group">
                <Menu.MenuItem key="fp-delete-msg" id="fp-delete-msg" label={hasDelete ? t("╪¡╪░┘ü ╪╣┘å╪»┘è (┘ê┘ç┘à┘è)", "Delete for me (fake)") : t("╪¡╪░┘ü ╪º┘ä╪▒╪│╪º┘ä╪⌐", "Delete message")} color="danger"
                    action={() => { deletedMessages.add(message.id); hideMessageInDOM(message.id); toast(t("╪¬┘à ╪¡╪░┘ü ╪º┘ä╪▒╪│╪º┘ä╪⌐ (┘à╪¡┘ä┘è)", "Message deleted (local)")); }} />
            </Menu.MenuGroup>
        ));
    } catch (e) {
    ...
    try {
        ...
        const groupA = (
            <Menu.MenuGroup key="fp-group-a">
                <Menu.MenuItem key="fp-rename" id="fp-rename" label={t("╪¬╪║┘è┘è╪▒ ╪º┘ä╪º╪│┘à ╪º┘ä┘à╪│╪¬╪╣╪º╪▒", "Change Nickname")} action={() => openModal(p => <RenameModal rootProps={p} user={user} guildId={guildId} />)} />
                <Menu.MenuItem key="fp-roles" id="fp-roles" label={t("╪º┘ä╪▒╪¬╪¿", "Roles")}>
                    {allRoles.length === 0
                        ? <Menu.MenuItem key="fp-roles-empty" id="fp-roles-empty" label={t("┘ä╪º ╪¬┘ê╪¼╪» ╪▒╪¬╪¿", "No roles")} disabled />
                        : [...allRoles.map(role => {
                        ...
                        <Menu.MenuItem key="fp-role-add" id="fp-role-add" label={t("+ ╪Ñ╪╢╪º┘ü╪⌐ ╪▒╪¬╪¿╪⌐", "+ Add a role")} action={() => openModal(p => <AddRoleModal rootProps={p} user={user} guildId={guildId} />)}
                            render={() => <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 10px", borderTop: "1px solid rgba(79,84,92,0.48)", color: "#b9bbbe", fontSize: 13, cursor: "pointer" }}><span>+</span><span>{t("╪Ñ╪╢╪º┘ü╪⌐ ╪▒╪¬╪¿╪⌐", "Add a role")}</span></div>} />,
                        ]}
                </Menu.MenuItem>
                <Menu.MenuItem key="fp-move" id="fp-move" label={t("╪º┘ä┘å┘é┘ä ╪Ñ┘ä┘ë", "Move to")}>
                    {(() => {
                        ...
                        if (allChannels.length === 0) return <Menu.MenuItem key="fp-move-empty" id="fp-move-empty" label={t("┘ä╪º ╪¬┘ê╪¼╪» ┘é┘å┘ê╪º╪¬ ╪╡┘ê╪¬┘è╪⌐", "No voice channels")} disabled />;
                        return allChannels.map(ch => <Menu.MenuItem key={`fp-move-${ch.id}`} id={`fp-move-${ch.id}`} label={`≡ƒöè ${ch.name}`} action={() => toast(t(`╪¬┘à ╪º┘ä┘å┘é┘ä ╪Ñ┘ä┘ë #${ch.name} ΓÇö ┘à╪¡╪º┘â╪º╪⌐`, `Moved to #${ch.name} ΓÇö simulation`))} />);
                    })()}
                </Menu.MenuItem>
                <Menu.MenuCheckboxItem key="fp-mute" id="fp-mute" label={t("┘â╪¬┘à ┘ü┘è ╪º┘ä╪«╪º╪»┘à", "Server Mute")} color="danger" checked={mutedUsers.get(user.id) === true} action={() => { const next = !mutedUsers.get(user.id); mutedUsers.set(user.id, next); applyVoiceBadges(); }} />
                <Menu.MenuCheckboxItem key="fp-deafen" id="fp-deafen" label={t("╪Ñ╪╡┘à╪º┘à ┘ü┘è ╪º┘ä╪«╪º╪»┘à", "Server Deafen")} color="danger" checked={deafenedUsers.get(user.id) === true} action={() => { const next = !deafenedUsers.get(user.id); deafenedUsers.set(user.id, next); applyVoiceBadges(); }} />
                <Menu.MenuItem key="fp-disconnect" id="fp-disconnect" label={t("┘é╪╖╪╣ ╪º┘ä╪º╪¬╪╡╪º┘ä", "Disconnect")} color="danger" action={() => { disconnectedUsers.add(user.id); notifyMemberListChange(); toast(t(`╪¬┘à ┘ü╪╡┘ä @${username} ┘à┘å ╪º┘ä╪╡┘ê╪¬ (┘à╪¡┘ä┘è)`, `@${username} disconnected from voice (local)`)); }} />
                <Menu.MenuItem key="fp-kick" id="fp-kick" label={t(`┘à┘ç┘ä╪⌐ ${username}`, `Timeout ${username}`)} color="danger" action={() => openModal(p => <TimeoutModal rootProps={p} user={user} />)} />
                <Menu.MenuItem key="fp-expulser" id="fp-expulser" label={t(`╪╖╪▒╪» ${username}`, `Kick ${username}`)} color="danger" action={() => openModal(p => <KickModal rootProps={p} user={user} guildId={guildId} />)} />
                <Menu.MenuItem key="fp-ban" id="fp-ban" label={t(`╪¡╪╕╪▒ ${username}`, `Ban ${username}`)} color="danger" action={() => openModal(p => <BanModal rootProps={p} user={user} />)} />
            </Menu.MenuGroup>
        );
...
// ΓöÇΓöÇΓöÇ Settings ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
const settings = definePluginSettings({
    enabled: {
        ...
        onChange(v: boolean) {
            ...
            toast(isEnabled ? t("╪¬┘à ╪¬┘ü╪╣┘è┘ä FakePerm Γ£ô", "FakePerm enabled Γ£ô") : t("╪¬┘à ╪¬╪╣╪╖┘è┘ä FakePerm Γ£ô", "FakePerm disabled Γ£ô"));
        }
    }
});
...
export default definePlugin({
    ...
    async start() {
        ...
        // Patches are ALWAYS registered ΓÇö they check isEnabled at runtime
