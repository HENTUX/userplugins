...
export default definePlugin({
    ...
    commands: [
        {
            name: "define",
            description: t("╪º╪¿╪¡╪½ ╪╣┘å ╪¬╪╣╪▒┘è┘ü ┘â┘ä┘à╪⌐", "Look up the definition of a word"),
            ...
            execute: async (opts, ctx) => {
                ...
                try {
                    ...
                    const entry = data[0];
                    const lines: string[] = [`≡ƒôû **${word}**${entry.phonetic ? ` ${entry.phonetic}` : ""}`];

                    for (const meaning of entry.meanings.slice(0, 3)) {
                        const def = meaning.definitions[0];
                        if (!def) continue;
                        lines.push(`> *${meaning.partOfSpeech}* ΓÇö ${def.definition}`);
                        if (def.example) lines.push(`> _e.g. ${def.example}_`);
                    }

                    sendBotMessage(ctx.channel.id, { content: lines.join("\n") });
                } catch {
                    sendBotMessage(ctx.channel.id, { content: t("╪¬╪╣╪░┘æ╪▒ ╪º┘ä┘ê╪╡┘ê┘ä ╪Ñ┘ä┘ë ╪º┘ä┘é╪º┘à┘ê╪│ ╪º┘ä╪ó┘å.", "Couldn't reach the dictionary right now.") });
                }
            }
        }
    ]
