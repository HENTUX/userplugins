...
async function save() {
    try {
        ...
        showToast(t("╪¬╪╣╪░┘æ╪▒ ╪¡┘ü╪╕ ╪º┘ä╪┤╪º╪▒╪⌐.", "Couldn't save the badge."), Toasts.Type.FAILURE);
    }
    notifyChanged();
}

function addBadge(name: string, src: string) {
    badges = [...badges, { id: `${Date.now()}`, name: name.trim() || t("╪┤╪º╪▒╪¬┘è", "My badge"), src }];
    void save();
}
...
async function pickFromDevice() {
    ...
    if (file.size > MAX_BYTES) {
        showToast(
            t("╪º┘ä╪╡┘ê╪▒╪⌐ ┘â╪¿┘è╪▒╪⌐ ╪¼╪»╪º┘ï (╪º┘ä╪¡╪» 2 ┘à┘è╪¼╪º╪¿╪º┘è╪¬).", "That image is too large (2 MB limit)."),
            Toasts.Type.FAILURE
        );
        return;
    }
    ...
    try {
        ...
        showToast(t("╪¬┘à╪¬ ╪Ñ╪╢╪º┘ü╪⌐ ╪º┘ä╪┤╪º╪▒╪⌐.", "Badge added."), Toasts.Type.SUCCESS);
    } catch (e) {
        logger.error("failed to read the picked image", e);
        showToast(t("╪¬╪╣╪░┘æ╪▒ ┘é╪▒╪º╪í╪⌐ ╪º┘ä╪╡┘ê╪▒╪⌐.", "Couldn't read that image."), Toasts.Type.FAILURE);
    }
}
...
function BadgeManager() {
    ...
    function addFromUrl() {
        ...
        if (!/^https:\/\/\S+$/i.test(trimmed)) {
            showToast(
                t("╪ú╪»╪«┘ä ╪▒╪º╪¿╪╖╪º┘ï ┘à╪¿╪º╪┤╪▒╪º┘ï ┘è╪¿╪»╪ú ╪¿┘Ç https://", "Enter a direct link starting with https://"),
                Toasts.Type.FAILURE
            );
            return;
        }
    ...
    return (
        <div className={cl("root")} dir={isArabicMode() ? "rtl" : "ltr"}>
            <div className={cl("note")}>
                {t(
                    "┘ç╪░┘ç ╪º┘ä╪┤╪º╪▒╪º╪¬ ┘à╪¡┘ä┘è╪⌐ ╪¬┘à╪º┘à╪º┘ï: ╪¬┘Å╪«╪▓┘Ä┘æ┘å ╪╣┘ä┘ë ╪¼┘ç╪º╪▓┘â ┘ê┘ä╪º ╪¬┘Å╪▒╪│┘Ä┘ä ╪Ñ┘ä┘ë ╪»┘è╪│┘â┘ê╪▒╪»╪î ┘ü┘ä╪º ┘è╪▒╪º┘ç╪º ╪ú╪¡╪» ╪║┘è╪▒┘â.",
            ...
            <div className={cl("add")}>
                <TextInput
                    value={name}
                    onChange={setName}
                    placeholder={t("╪º╪│┘à ╪º┘ä╪┤╪º╪▒╪⌐ (┘è╪╕┘ç╪▒ ╪╣┘å╪» ┘à╪▒┘ê╪▒ ╪º┘ä┘à╪ñ╪┤╪▒)", "Badge name (shown on hover)")}
                />
                <div className={cl("row")}>
                    <TextInput
                        value={url}
                        onChange={setUrl}
                        placeholder={t("╪▒╪º╪¿╪╖ ┘à╪¿╪º╪┤╪▒ ┘ä┘ä╪╡┘ê╪▒╪⌐ https://...", "Direct image link https://...")}
                    />
                    <Button color={Button.Colors.BRAND} onClick={addFromUrl}>
                        {t("┘à┘å ╪▒╪º╪¿╪╖", "From link")}
                    </Button>
                    <Button color={Button.Colors.PRIMARY} onClick={() => void pickFromDevice()}>
                        {t("┘à┘å ╪¼┘ç╪º╪▓┘è", "From device")}
                    </Button>
                </div>
            </div>

            <Text variant="text-sm/semibold" className={cl("label")}>
                {t(`╪┤╪º╪▒╪º╪¬┘è (${list.length})`, `My badges (${list.length})`)}
            </Text>

            {list.length === 0 ? (
                <div className={cl("empty")}>
                    {t("┘ä╪º ╪¬┘ê╪¼╪» ╪┤╪º╪▒╪º╪¬ ╪¿╪╣╪».", "No badges yet.")}
                </div>
            ) : (
...
export default definePlugin({
    ...
    userProfileBadges: [{
        id: "vc-mybadges",
        key: "MyBadges",
        description: t("╪┤╪º╪▒╪⌐ ┘à╪¡┘ä┘è╪⌐", "Local badge"),
