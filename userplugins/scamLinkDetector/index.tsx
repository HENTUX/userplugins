...
function checkForScamLinks(content: string): string[] {
    ...
    for (const domain of domains) {
        if (scamLinks.has(domain)) {
            ...
            if (settings.store.enableDebugLogs) {
                logger.debug(`Γ£ô ${domain} is not in scam database`);
            }
        }
    }
...
export default definePlugin({
    ...
    flux: {
        async MESSAGE_CREATE({ optimistic, type, message, channelId }: IMessageCreate) {
            ...
            logger.warn(`≡ƒÜ¿ SCAM LINKS DETECTED! Found ${scamDomains.length} scam domain(s): ${scamDomains.join(", ")}`);
            ...
            const warningMessage = t(
                `ΓÜá∩╕Å **╪▒╪º╪¿╪╖ ╪º╪¡╪¬┘è╪º┘ä┘è ┘à┘â╪¬╪┤┘Ä┘ü**\n\n╪▒╪│╪º┘ä╪⌐ **${message.author.username}** ╪¬╪¡╪¬┘ê┘è ╪▒┘ê╪º╪¿╪╖ ╪º╪¡╪¬┘è╪º┘ä/╪«╪¿┘è╪½╪⌐ ┘à╪╣╪▒┘ê┘ü╪⌐:\n${domainList}\n\n┘ç╪░┘ç ╪º┘ä┘å╪╖╪º┘é╪º╪¬ ┘à┘Å╪╣┘ä┘Ä┘æ┘à╪⌐ ┘ü┘è ┘é╪º╪╣╪»╪⌐ Discord AntiScam. ┘ä╪º ╪¬╪╢╪║╪╖ ╪╣┘ä┘è┘ç╪º!`,
                `ΓÜá∩╕Å **Scam Link Detected**\n\nThis message from **${message.author.username}** contains known scam/malicious links:\n${domainList}\n\nThese domains are flagged in the Discord AntiScam database. Do not click them!`
            );
