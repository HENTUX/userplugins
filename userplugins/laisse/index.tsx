...
async function moveUserToVoiceChannel(
    ...
    try {
        ...
        if (settings.store.showNotifications) {
            ...
            showNotification({
                title: t("Leash - Ù†Ø¬Ø§Ø­", "Leash - Success"),
                body: t(`ØªÙ… Ù†Ù‚Ù„ ${user?.username || "Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…"} Ø¥Ù„Ù‰ Ù‚Ù†Ø§ØªÙƒ Ø§Ù„ØµÙˆØªÙŠØ©`, `${user?.username || "User"} has been moved to your voice channel`),
            });
        }
    } catch (error) {
...
const UserContextMenuPatch: NavContextMenuPatchCallback = (
    ...
    children.push(
        ...
        <Menu.MenuCheckboxItem
            ...
            label={t("Leash - Ø§Ø±Ø¨Ø· Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…", "Leash - Hook the user")}
            checked={checked}
            action={() => {
                if (leashedUserInfo?.userId === user.id) {
                    ...
                    showNotification({
                        title: "Leash",
                        body: t(`Ù„Ù… ÙŠØ¹ÙØ¯ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù… ${user.username} Ù…Ø±Ø¨ÙˆØ·Ø§Ù‹`, `User ${user.username} is no longer hooked`),
                    });
                    return;
                }
                ...
                showNotification({
                    title: "Leash",
                    body: t(`Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù… ${user.username} Ù…Ø±Ø¨ÙˆØ· Ø¨Ùƒ Ø§Ù„Ø¢Ù†`, `User ${user.username} is now hooked to you`),
                });
            }}
        />
    );
...
export default definePlugin({
    name: "Leash",
    description: "Leashes a user to you by automatically moving them to whatever voice channel you join.\n\nâš ï¸ WARNING: Moving users to voice channels without their consent may violate AtlasXGOD's Terms of Service and community guidelines. This feature requires server moderation permissions. Use responsibly.",
    tags: ["Utility"],
    ...
    settingsAboutComponent: () => (
        <div style={{
            ...
            {t("âš ï¸ ØªØ­Ø°ÙŠØ±: Ù†Ù‚Ù„ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…ÙŠÙ† Ø¯ÙˆÙ† Ù…ÙˆØ§ÙÙ‚ØªÙ‡Ù… Ù‚Ø¯ ÙŠØ®Ø§Ù„Ù Ø´Ø±ÙˆØ· Ø®Ø¯Ù…Ø© Discord. ØªØªØ·Ù„Ø¨ ØµÙ„Ø§Ø­ÙŠØ§Øª Ø¥Ø´Ø±Ø§Ù ÙÙŠ Ø§Ù„Ø³ÙŠØ±ÙØ±. Ø§Ø³ØªØ®Ø¯Ù…Ù‡Ø§ Ø¨Ù…Ø³Ø¤ÙˆÙ„ÙŠØ©.", "âš ï¸ WARNING: Moving users without their consent may violate Discord's Terms of Service. Requires server moderation permissions. Use responsibly.")}
        </div>
    ),
    settings,
    contextMenus: {
                        ...
                        if (
                            ...
                            try {
                                ...
                                if (settings.store.showNotifications) {
                                    showNotification({
                                        title: "Leash",
                                        body: t(`Ù…Ø­Ø§ÙˆÙ„Ø© Ù†Ù‚Ù„ ${user?.username || "Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù…"} Ø¥Ù„Ù‰ Ù‚Ù†Ø§ØªÙƒ Ø§Ù„ØµÙˆØªÙŠØ©`, `Attempting to move ${user?.username || "user"} to your voice channel`),
                                    });
                                }
                                ...
                                if (settings.store.showNotifications) {
                                    showNotification({
                                        title: t("Leash - Ø®Ø·Ø£", "Leash - Error"),
                                        body: t("ØªØ¹Ø°Ù‘Ø± Ù†Ù‚Ù„ Ø§Ù„Ù…Ø³ØªØ®Ø¯Ù… (ØµÙ„Ø§Ø­ÙŠØ§Øª ØºÙŠØ± ÙƒØ§ÙÙŠØ©)", "Unable to move user (insufficient permissions)"),
                                    });
                                }
                            }
                        }
