...
import { ModalCloseButton, ModalContent, ModalHeader, ModalRoot, RenderModalProps } from "@utils/esharqModals";
        ...
        >
            ...
            <div className={cl("result-body")}>
                <div className={cl("result-header")}>
                    ...
                    {message.pinned && <span className={cl("result-pin")}>{t("┘à╪½╪¿┘æ╪¬╪⌐", "Pinned")}</span>}
...
export function DeepSearchModal({ rootProps }: { rootProps: RenderModalProps; }) {
    ...
    const exportResults = useCallback(async () => {
        if (results.length === 0) {
            showNotification({ title: "Deep Search", body: t("┘ä╪º ╪¬┘ê╪¼╪» ┘å╪¬╪º╪ª╪¼ ╪¿╪¡╪½ ┘ä┘ä╪¬╪╡╪»┘è╪▒.", "No search results to export.") });
            return;
        }
        ...
        try {
            ...
            showNotification({ title: "Deep Search", body: t("╪¬╪╣╪░┘æ╪▒ ╪¬╪╡╪»┘è╪▒ ┘å╪¬╪º╪ª╪¼ ╪º┘ä╪¿╪¡╪½.", "Failed to export search results.") });
        }
    ...
    return (
        <ModalRoot {...rootProps} size={ModalSize.LARGE} className={cl("modal-root")}>
            <ModalHeader className={cl("header")}>
                <span className={cl("header-title")}>{t("╪º┘ä╪¿╪¡╪½ ╪º┘ä╪╣┘à┘è┘é", "Deep Search")}</span>
                {currentGuildId && (
                ...
                <div className={cl("container")}>
                    <div className={cl("search-section")}>
                        <div className={cl("search-bar")}>
                            ...
                            <input
                                ...
                                type="text"
                                value={query}
                                placeholder={t("╪º╪¿╪¡╪½ ┘ü┘è ╪º┘ä╪▒╪│╪º╪ª┘ä...", "Search messages...")}
                        ...
                        {showAdvanced && (
                            <div className={cl("advanced-filters")}>
                                <div className={cl("advanced-row")}>
                                    <FilterInput
                                        label={t("┘à┘å ┘à╪╣╪▒┘æ┘ü ┘à╪│╪¬╪«╪»┘à", "From user ID")}
                                        value={filters.authorId || ""}
                                        placeholder={t("┘à╪╣╪▒┘æ┘ü ╪º┘ä┘à╪│╪¬╪«╪»┘à", "User ID")}
                                        onChange={v => updateFilter("authorId", v || null)}
                                    />
                                    <FilterInput
                                        label={t("┘ü┘è ┘à╪╣╪▒┘æ┘ü ┘é┘å╪º╪⌐", "In channel ID")}
                                        value={filters.channelId || ""}
                                        placeholder={t("┘à╪╣╪▒┘æ┘ü ╪º┘ä┘é┘å╪º╪⌐", "Channel ID")}
                                        onChange={v => updateFilter("channelId", v || null)}
                                    />
                                    <FilterInput
                                        label={t("┘è╪░┘â╪▒ ┘à╪╣╪▒┘æ┘ü ┘à╪│╪¬╪«╪»┘à", "Mentions user ID")}
                                ...
                                <div className={cl("advanced-row")}>
                                    <FilterInput
                                        label={t("╪¿╪╣╪» ╪¬╪º╪▒┘è╪«", "After date")}
                                        value={filters.dateFrom || ""}
                                        placeholder="YYYY-MM-DD"
                                        onChange={v => updateFilter("dateFrom", v || null)}
                                    />
                                    <FilterInput
                                        label={t("┘é╪¿┘ä ╪¬╪º╪▒┘è╪«", "Before date")}
                    ...
                    <div className={cl("results-header")}>
                        {loading ? (
                            ...
                            <span className={cl("results-stats")}>{t("┘ä╪º ╪¬┘ê╪¼╪» ┘å╪¬╪º╪ª╪¼", "No results found")}</span>
                        ) : (
                            <span className={cl("results-stats")}>{t("╪º┘â╪¬╪¿ ╪º╪│╪¬╪╣┘ä╪º┘à╪º┘ï ╪ú┘ê ╪º╪«╪¬╪▒ ┘à╪▒╪┤┘æ╪¡╪º╪¬ ┘ä┘ä╪¿╪¡╪½", "Type a query or select filters to search")}</span>
                        )}
                        {results.length > 0 && (
