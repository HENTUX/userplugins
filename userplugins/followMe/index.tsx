...
// already warned about and only speak up when that changes ΓÇö otherwise a missing
...
async function moveTargetTo(guildId: string, channelId: string) {
    ...
    if (!PermissionStore.can(PermissionsBits.MOVE_MEMBERS, channel)) {
        if (lastFailedChannel !== channelId) {
            ...
            notify(t(
                `╪¬╪╣╪░┘æ╪▒ ┘å┘é┘ä ${targetName}: ┘ä╪º ╪¬┘à┘ä┘â ╪╡┘ä╪º╪¡┘è╪⌐ ┬½┘å┘é┘ä ╪º┘ä╪ú╪╣╪╢╪º╪í┬╗ ┘ü┘è ┘ç╪░┘ç ╪º┘ä┘é┘å╪º╪⌐`,
    ...
    try {
        ...
        notify(t(`┘å┘Å┘é┘ä ${targetName} ╪Ñ┘ä┘ë ┘é┘å╪º╪¬┘â`, `Moved ${targetName} to your channel`));
    } catch (e) {
        console.error("[FollowMe] Failed to move user:", e);
        if (lastFailedChannel !== channelId) {
            lastFailedChannel = channelId;
            notify(t(`╪¬╪╣╪░┘æ╪▒ ┘å┘é┘ä ${targetName}`, `Failed to move ${targetName}`), true);
        }
    }
}
...
function follow(userId: string) {
    ...
    notifyAll();
    persist();
    Toasts.show({ message: t(`┘è╪¬╪¿╪╣┘â ╪º┘ä╪ó┘å: ${targetName}`, `Now following you: ${targetName}`), type: Toasts.Type.SUCCESS, id: Toasts.genId() });
}
...
function unfollow() {
    ...
    notifyAll();
    persist();
    Toasts.show({ message: t(`╪¬┘à┘æ ╪Ñ┘è┘é╪º┘ü ╪Ñ╪¼╪¿╪º╪▒ ${name} ╪╣┘ä┘ë ╪º┘ä┘à╪¬╪º╪¿╪╣╪⌐`, `Stopped forcing ${name} to follow`), type: Toasts.Type.MESSAGE, id: Toasts.genId() });
}
...
function StopHeaderButton() {
    ...
    return (
        <HeaderBarButton
            icon={StopIcon}
            tooltip={t(`╪Ñ┘è┘é╪º┘ü ╪º┘ä┘à╪¬╪º╪¿╪╣╪⌐: ${targetName}`, `Stop Follow Me: ${targetName}`)}
            onClick={unfollow}
        />
    );
}
...
const ctxPatch: NavContextMenuPatchCallback = (children, props) => {
    ...
    children.push(
        <Menu.MenuCheckboxItem
            id="esharq-follow-me"
            label={isFollowed ? t("╪Ñ┘è┘é╪º┘ü ╪º┘ä┘à╪¬╪º╪¿╪╣╪⌐", "Stop Follow Me") : t("╪º╪¼╪╣┘ä┘ç ┘è╪¬╪¿╪╣┘å┘è", "Follow Me")}
            checked={isFollowed}
...
const settings = definePluginSettings({
    warning: {
        ...
        component: () => <BanRiskWarning
            ar="╪¬╪¡╪░┘è╪▒: ┘å┘é┘ä ╪º┘ä┘à╪│╪¬╪«╪»┘à┘è┘å ╪»┘ê┘å ┘à┘ê╪º┘ü┘é╪¬┘ç┘à ┘é╪» ┘è╪«╪º┘ä┘ü ╪┤╪▒┘ê╪╖ ╪«╪»┘à╪⌐ Discord ┘ê┘é╪» ┘è╪╣╪▒┘æ╪╢ ╪¡╪│╪º╪¿┘â ┘ä┘ä╪¡╪╕╪▒. ┘è╪¬╪╖┘ä┘æ╪¿ ╪╡┘ä╪º╪¡┘è╪⌐ ┬½┘å┘é┘ä ╪º┘ä╪ú╪╣╪╢╪º╪í┬╗ ┘ü┘è ╪º┘ä╪│┘è╪▒┘ü╪▒. ╪º╪│╪¬╪«╪»┘à┘ç╪º ╪¿┘à╪│╪ñ┘ê┘ä┘è╪⌐."
...
export default definePlugin({
    name: "FollowMe",
    description: "Forces a user to follow you between voice channels when you have the Move Members permission. Right-click a user ΓåÆ Follow Me.",
