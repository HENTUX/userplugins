...
const settings = definePluginSettings({
    ...
    temperature: {
        ...
        description: "Temperature ΓÇö 0 = precise, 1 = creative.",
    ...
    commands: [
        {
            ...
            execute: async (opts, ctx) => {
                ...
                if (!channel) {
                    sendBotMessage(ctx.channel.id, { content: t("┘é┘å╪º╪⌐ ╪║┘è╪▒ ╪╡╪º┘ä╪¡╪⌐.", "Invalid channel specified.") });
                    return;
                }
                ...
                sendBotMessage(ctx.channel.id, { content: t(`> ╪¼╪º╪▒┘è ╪¼┘ä╪¿ ╪º┘ä╪▒╪│╪º╪ª┘ä ┘à┘å #${channelName}...`, `> Fetching messages from #${channelName}...`) });

                try {
                    const messages = await fetchMessages(channelId, msgLimit, timeframe, personId || undefined);

                    if (messages.length === 0) {
                        sendBotMessage(ctx.channel.id, { content: t(`> ┘ä╪º ╪¬┘ê╪¼╪» ╪▒╪│╪º╪ª┘ä ┘ü┘è #${channelName}.`, `> No messages found in #${channelName}.`) });
                        return;
                    }

                    sendBotMessage(ctx.channel.id, { content: t(`> ╪╣┘Å╪½╪▒ ╪╣┘ä┘ë ${messages.length} ╪▒╪│╪º┘ä╪⌐. ╪¼╪º╪▒┘è ╪º┘ä╪¬┘ä╪«┘è╪╡...`, `> Found ${messages.length} messages. Summarizing...`) });
                    ...
                    sendBotMessage(ctx.channel.id, { content: t(`**┘à┘ä╪«┘æ╪╡ #${channelName}** (${messages.length} ╪▒╪│╪º┘ä╪⌐):\n\n${summary}`, `**Summary of #${channelName}** (${messages.length} messages):\n\n${summary}`) });
                } catch (err: any) {
                    sendBotMessage(ctx.channel.id, { content: t(`> ╪«╪╖╪ú: ${err?.message || "╪¬╪╣╪░┘æ╪▒ ╪Ñ┘å╪┤╪º╪í ╪º┘ä┘à┘ä╪«┘æ╪╡"}`, `> Error: ${err?.message || "Failed to generate summary"}`) });
                }
            }
        }
    ]
