    ...
    try {
        ...
        reader.onloadend = async () => {
            ...
            try {
                ...
                Toasts.show(Toasts.create(t(`╪¬┘à ╪¬╪¡╪»┘è╪½ ${type} ╪¿┘å╪¼╪º╪¡!`, `Successfully updated your ${type}!`), Toasts.Type.SUCCESS));
            } catch (error) {
                Toasts.show(Toasts.create(t(`┘ü╪┤┘ä ╪¬╪¡╪»┘è╪½ ${type}.`, `Failed to update ${type}.`), Toasts.Type.FAILURE));
                console.error("FastPFP Error:", error);
            }
        };
    } catch (err) {
        Toasts.show(Toasts.create(t("┘ü╪┤┘ä ╪¬┘å╪▓┘è┘ä ╪º┘ä╪╡┘ê╪▒╪⌐.", "Failed to download image."), Toasts.Type.FAILURE));
...
const messageContextMenuPatch: NavContextMenuPatchCallback = (children, props) => {
    ...
    if (group && !group.some(child => child?.props?.id === "fastpfp-avatar")) {
        group.push(
            <Menu.MenuItem
                label={t("╪Ñ╪╢╪º┘ü╪⌐ ╪Ñ┘ä┘ë ╪º┘ä╪╡┘ê╪▒╪⌐ ╪º┘ä╪┤╪«╪╡┘è╪⌐", "Add To PFP")}
                key="fastpfp-avatar"
                id="fastpfp-avatar"
        ...
        group.push(
            <Menu.MenuItem
                label={t("╪Ñ╪╢╪º┘ü╪⌐ ╪Ñ┘ä┘ë ╪º┘ä╪¿╪º┘å╪▒", "Add To Banner")}
                key="fastpfp-banner"
                id="fastpfp-banner"
